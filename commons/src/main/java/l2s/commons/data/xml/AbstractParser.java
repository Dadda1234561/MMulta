...
public abstract class AbstractParser<H extends AbstractHolder> extends LoggerObject
{
	...
	public <T extends Enum<T>> T parseEnum(Node node, Class<T> clazz, T defaultValue)
	{
		...
		try {
			return Enum.valueOf(clazz, node.getStringValue());
		} catch (IllegalArgumentException e) {
			_log.info("Invalid value specified for node: " + node.getName() + " specified value: " + node.getStringValue() + " should be enum value of \"" + clazz.getSimpleName() + "\" using default value: " + defaultValue);
			return defaultValue;
		}
	}
	...
	public double parseFloat(Element element, String param, float defaultValue) {
		String value = element.attributeValue(param);
		if(value == null)
			return defaultValue;
	...
	public double parseFloat(Element element, String param) {
		return Float.parseFloat(element.attributeValue(param));
	}
