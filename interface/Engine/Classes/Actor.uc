...
// collect bool flags cause memory size.
...
// flagoftiger	Spawn???? true?? ????.
var bool				bNeedPostSpawnProcess;

//#ifdef L2_NEW_PAWN flagoftiger
var bool				bReadyToRender;
...
//#ifdef L2_NEW_PAWN flagoftiger
var	transient protected MeshContainer	MeshContainer;
//#endif
...
// __L2 gigadeth
var(Display) float			RangeForIgnoredRange;	// bIgnoredRage ?? true ?? ???? ???? ?????? ?????? 
...
// flagoftiger
var			  int	      KeyboardRotationYawFromServer;	// ???????? ???? ????????????
...
var	transient vector		ColorScaleFactor; //???? ?????? ????	

// jylee, ???? ???? ???? ???????? ???????? ????. 121019
// var bool					bUseColorScaleBackUp; // ?????? ????????
//  var	transient vector		ColorScaleFactorBackUp; // ?????? ????????
//  var transient bool			bAttachedToSkeletalMeshBone;
//   var transient SkeletalMeshInstance Attac;
...
// #if defined(__L2)	// 2008/01/09 NeverDie
...
//#ifdef	__L2	Hunter
	...
	optional bool	  bNeedMeshContainer
//#endif
);
...
//#ifdef __L2 // ttmayrin
native final function PlayLoopSound
(
	sound				Sound,
	optional ESoundSlot Slot,
	optional float		Volume,
	...
	optional float		Pitch,
	optional bool		Attenuate,
	optional float		FadeInDuration
);
native final function StopSound( Sound a_Sound, optional float a_StopByFadeOutTime );
//#endif
...
// NPC?? ???? Mesh ?? SetPawnResource ???? ????????
// ?????? PostBeginPlay() ???? ???????? ???? Attach?? ?????? Mesh?? ???? ?????? Pawn?? ?????? ???????? ???? ?????? ??????.
// ?????????? uc ?????????? ???????? ?????? ???? ?? ???? ???? ?????? ?? ?????? ????????. - by idearain [2006/11/06]
...
function NextScene( Name EventName, bool bReverse );
...
event NextSceneEvent( Name CurEventName, Name NextEventName, bool bReverse  )
{
	local Actor A;
	...
	ForEach DynamicActors( class 'Actor', A, CurEventName )
		A.NextScene(NextEventName, bReverse);
}
...
defaultproperties
{
    ...
    L2ActorViewDuration=2.5
    L2LodViewDuration=1.5
    L2CurrentLod=-1
