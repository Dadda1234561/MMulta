...
function OnEvent(int Event_ID, string param)
{
	...
	local int AddSuccRate;
	local int CreateCritical;
...
function AddRecipeShopSellItem(int RecipeID, int CanbeMade, INT64 MakingFee, int AddSuccRate, int CreateCritical )
{
	...
	// 성공 확률
	successRate = class'UIDATA_RECIPE'.static.GetRecipeSuccessRate(RecipeID);
	...
	infNodeItem = infNodeItemClear;
	infNodeItem.eType = XTNITEM_TEXT;
	infNodeItem.t_strText = successRate $ "%";
	infNodeItem.t_bDrawOneLine = true;
	infNodeItem.nOffSetX = 0;
	infNodeItem.nOffSetY = -8;
	...
	{	
		...
		if ( successRate != 100 ) 
		{
			...
			if ( successRate + AddSuccRate <= 100 ) 
				infNodeItem.t_strText = " +" $ AddSuccRate $"%";
			else 
				infNodeItem.t_strText = " +" $ 100 - successRate $"%";
		...
		// -1인 경우 레시피에 크리티컬 확률이 아예 없는 경우이다.
		if ( CreateCritical > -1 ) 
		{
			//Insert Node Item - "크리티컬 "
			infNodeItem = infNodeItemClear;
			infNodeItem.eType = XTNITEM_TEXT;
			...
			if ( CreateCritical == 0 ) 
				...
				infNodeItem.t_strText = CreateCritical $ "%";
