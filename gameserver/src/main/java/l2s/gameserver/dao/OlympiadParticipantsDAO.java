...
public class OlympiadParticipantsDAO
{
	...
	private static final String SELECT_SQL_QUERY = "SELECT char_id, characters.char_name as char_name, character_dualclasses.class_id, olympiad_points, olympiad_points_past, olympiad_points_past_static, competitions_done, competitions_loose, competitions_win, game_classes_count, game_noclasses_count FROM olympiad_participants LEFT JOIN characters ON characters.obj_Id = olympiad_participants.char_id LEFT JOIN character_dualclasses ON character_dualclasses.char_obj_id = olympiad_participants.char_id AND character_dualclasses.type=? WHERE characters.obj_Id > 0 AND characters.is_noble > 0";
	...
	public static final String OLYMPIAD_GET_HEROS = "SELECT `char_id`, characters.char_name AS char_name FROM `olympiad_participants` LEFT JOIN characters ON char_id=characters.obj_Id LEFT JOIN character_dualclasses ON char_id=character_dualclasses.char_obj_id AND character_dualclasses.type=? WHERE characters.obj_Id > 0 AND characters.is_noble > 0 AND character_dualclasses.class_id = ? AND `competitions_done` >= ? AND `competitions_win` > 0 ORDER BY `olympiad_points` DESC, `competitions_win` DESC, `competitions_done` DESC";
	public static final String GET_EACH_CLASS_LEADER = "SELECT characters.char_name AS char_name FROM `olympiad_participants` LEFT JOIN characters ON char_id=characters.obj_Id LEFT JOIN character_dualclasses ON char_id=character_dualclasses.char_obj_id AND character_dualclasses.type=? WHERE characters.obj_Id > 0 AND characters.is_noble > 0 AND character_dualclasses.class_id = ? AND `olympiad_points_past_static` != 0 ORDER BY `olympiad_points_past_static` DESC LIMIT 10";
