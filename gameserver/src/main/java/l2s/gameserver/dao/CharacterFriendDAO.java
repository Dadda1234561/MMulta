...
public class CharacterFriendDAO
{
	...
	public TIntObjectMap<Friend> select(Player owner)
	{
		...
		try
		{
			...
			statement = con.prepareStatement("SELECT f.friend_id, f.memo, c.char_name, s.class_id, s.level, c.clanid, clan.ally_id, clan.name, ally.ally_name, c.createtime, c.lastAccess FROM character_friends f LEFT JOIN characters c ON f.friend_id = c.obj_Id LEFT JOIN clan_data clan ON c.clanid = clan.clan_id LEFT JOIN ally_data ally ON clan.ally_id = ally.ally_id LEFT JOIN character_dualclasses s ON (f.friend_id = s.char_obj_id AND s.active = 1) WHERE f.char_id = ?");
			...
			while(rset.next())
			{
				...
				String clanName = rset.getString("clan.name");
