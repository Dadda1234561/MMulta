...
public final class RestorationInfo
{
	...
	private final boolean _isLimitUse;

	public RestorationInfo(int itemConsumeId, int itemConsumeCount, int onFailMessage, boolean isLimitUse)
	{
		_itemConsumeId = itemConsumeId;
		_itemConsumeCount = itemConsumeCount;
		_isLimitUse = isLimitUse;
	...
	public List<RestorationItem> getRandomGroupItems(ItemInstance cube)
	{
		final List<RestorationGroup> validGroups = new ArrayList<>();
		for(RestorationGroup group : _restorationGroups) {
			if (_isLimitUse && (cube != null) && (group.getId() != -1) && group.getId() != (cube.getUsedCnt() + 1)) {
				continue;
			}
		...
		for (RestorationGroup restorationGroup : validGroups) {
			chancesAmount += restorationGroup.getChance();
		}

		if(Rnd.chance(chancesAmount))
		{
			double chanceMod = (100. - chancesAmount) / validGroups.size();
			List<RestorationGroup> successGroups = new ArrayList<RestorationGroup>();
			int tryCount = 0;
			while(successGroups.isEmpty())
			{
				tryCount++;
				for(RestorationGroup group : validGroups)
				{
