	...
	public int loadDelayed(Player player, boolean notify)
	{
		...
		synchronized (_lock)
		{
			try
			{
				...
				ItemInstance item, newItem, additionalItem = null, addedAddItem = null;
					...
					if(ITEM_TEMPLATE != null)
					{
						...
						for(int i = 0; i < (stackable ? 1 : ITEM_COUNT); i++)
						{
							if(ITEM_COUNT > 0)
							{
								...
								if (ITEM_ID == 4037) {
									additionalItem = ItemFunctions.createItem(70065);
									if(additionalItem.isStackable()) {
										additionalItem.setCount(ITEM_COUNT);
									}
									else {
										additionalItem.setEnchantLevel(ITEM_ENCHANT);
									}

									additionalItem.setLocation(ItemLocation.INVENTORY);
									addedAddItem = inv.addItem(additionalItem);
									if (addedAddItem == null)
									{
										_log.warn("Unable to delayed create item " + ITEM_ID + " request " + PAYMENT_ID);
										continue;
									}
									...
									if(notify)
										player.sendPacket(SystemMessagePacket.obtainItems(70065, stackable ? ITEM_COUNT : 1, ITEM_ENCHANT));
								}

								Log.LogItem(player, Log.DelayedItemReceive, newItem, ITEM_COUNT, DESCRIPTION);
								if (ITEM_ID == 4037) {
									Log.LogItem(player, Log.DelayedItemReceive, additionalItem, ITEM_COUNT, "Additional Reward for Coin of Luck!");
								}
							}
