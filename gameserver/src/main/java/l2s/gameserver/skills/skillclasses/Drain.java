	...
	protected void useSkill(Creature activeChar, Creature target, boolean reflected)
	{
		...
		if(!corpseSkill)
		{
			...
			if(isMagic())
			{
				...
				realTarget.reduceCurrentHp(info.damage, activeChar, this, true, true, false, true, false, false, true, true, info.crit, info.miss, info.shld, false);
				if(info.damage >= 1)
				{
			...
			else
			{
				...
				if(info != null)
				{
					realTarget.reduceCurrentHp(info.damage, activeChar, this, true, true, false, true, false, false, true, true, info.crit || info.blow, info.miss, info.shld, false);
