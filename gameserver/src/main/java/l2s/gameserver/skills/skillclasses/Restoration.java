...
public class Restoration extends Skill
{
	...
	public void onEndCast(final Creature activeChar, final Set<Creature> targets)
	{
		...
		ItemInstance cubeItem = null;
		if (_restoration.isLimitUse())
		{
			if (player == null)
			{
				return;
			...
			cubeItem = player.getInventory().getItemByItemId(itemConsumeId);
		}

		final List<RestorationItem> restorationItems = _restoration.getRandomGroupItems(cubeItem);
		...
		if (_restoration.isLimitUse())
		{
            ...
            {
                // update item use
                cubeItem.setUsedCnt(cubeItem.getUsedCnt() + 1);
                cubeItem.setJdbcState(JdbcEntityState.UPDATED);
                cubeItem.update();

                final int maxUses = cubeItem.getTemplate().getMaxUseCnt();
                if (maxUses > 0 && cubeItem.getUsedCnt() >= maxUses)
                {
                    ItemFunctions.deleteItem(playable, cubeItem, cubeItem.getCount(), true);
                }
                else
                {
                    InventoryUpdatePacket iu = new InventoryUpdatePacket();
                    iu.addModifiedItem(player, cubeItem);
