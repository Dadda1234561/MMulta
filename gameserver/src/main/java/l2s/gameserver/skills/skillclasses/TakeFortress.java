...
public class TakeFortress extends Skill
{
	...
	public boolean checkCondition(SkillEntry skillEntry, Creature activeChar, Creature target, boolean forceUse, boolean dontMove, boolean first, boolean sendMsg, boolean trigger)
	{
		...
		if(first)
		{
			...
			{
				if(ch.getSkillCast(SkillCastingType.NORMAL).isCastingNow() && ch.getSkillCast(SkillCastingType.NORMAL).getSkillEntry().getTemplate().equals(this)) // проверяел ли ктото возле нас кастует накойже скил
				{
					activeChar.sendPacket(SystemMsg.A_FLAG_IS_ALREADY_BEING_DISPLAYED_ANOTHER_FLAG_CANNOT_BE_DISPLAYED);
					return false;
				}

				if(ch.getSkillCast(SkillCastingType.NORMAL_SECOND).isCastingNow() && ch.getSkillCast(SkillCastingType.NORMAL_SECOND).getSkillEntry().getTemplate().equals(this)) // проверяел ли ктото возле нас кастует накойже скил
				{
					activeChar.sendPacket(SystemMsg.A_FLAG_IS_ALREADY_BEING_DISPLAYED_ANOTHER_FLAG_CANNOT_BE_DISPLAYED);
					return false;
				}
