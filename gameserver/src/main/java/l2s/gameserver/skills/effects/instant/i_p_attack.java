...
public class i_p_attack extends i_abstract_effect
{
	...
	private final boolean _isForPlayer;
	...
	public i_p_attack(EffectTemplate template)
	{
		...
		_isForPlayer = getParams().getBool("damageForPlayerOnly", false);
	}
	...
	public void instantUse(Creature effector, Creature effected, boolean reflected)
	{
		...
		if (_static && _isForPlayer && !effected.isPlayable())
		{
			return;
		}
		if(_static)
		{
			realTarget.reduceCurrentHp(power, effector, getSkill(), true, true, _directHp, true, false, false, power != 0, true, false, false, false, true);
			return;
		}
		...
		realTarget.reduceCurrentHp(info.damage, effector, getSkill(), true, true, _directHp, true, false, false, getTemplate().isInstant() && power != 0, getTemplate().isInstant(), info.crit || info.blow, false, false, false);
