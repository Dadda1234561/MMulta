...
public final class EffectDamageBlock extends EffectHandler
{
	...
	public void onStart(Abnormal abnormal, Creature effector, Creature effected)
	{
		...
		if(_withException)
		{
			...
			{
				if(effector.getSkillCast(SkillCastingType.NORMAL).isCastingNow() && getSkill().equals(effector.getSkillCast(SkillCastingType.NORMAL).getSkillEntry().getTemplate()))
					effected.setDamageBlockedException(effector.getSkillCast(SkillCastingType.NORMAL).getTarget());
				else if(effector.getSkillCast(SkillCastingType.NORMAL).isCastingNow() && getSkill().equals(effector.getSkillCast(SkillCastingType.NORMAL_SECOND).getSkillEntry().getTemplate()))
