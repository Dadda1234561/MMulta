	...
	public boolean useAdminCommand(Enum<?> comm, String[] wordList, String fullString, Player activeChar)
	{
		...
		else if(fullString.startsWith("admin_list_spawns"))
		{
			...
			try
			{
				...
				npcId = Integer.parseInt(args[1].trim());
				if (args.length > 2)
				{
					teleportIndex = Integer.parseInt(args[2].trim());
				}
			}
			catch(Exception NumberFormatException)
			{
				...
				if(npcInst.isVisible())
				{
					...
					if(teleportIndex > -1)
					{
						if(teleportIndex == index)
							activeChar.teleToLocation(npcInst.getLoc(), npcInst.getReflection());
					}
					else
						activeChar.sendMessage(index + " - " + npcInst.getName() + " (" + npcInst.getObjectId() + "): " + npcInst.getX() + " " + npcInst.getY() + " " + npcInst.getZ() + " [ref=" + npcInst.getReflectionId()+"]");
				}
			}
		}
	}
