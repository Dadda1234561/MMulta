		...
		switch(command)
		{
			...
			case admin_spawnkill:
				...
				try
				{
					...
					SimpleSpawner npcSpawn = new SimpleSpawner(id);
					npcSpawn.setSpawnRange((geoIndex, fly) -> activeChar.getLoc());
					npcSpawn.setAmount(1);
					npcSpawn.setRespawnDelay(0);
					npcSpawn.setReflection(activeChar.getReflection());

					for (int i = 0; i < cnt; i++)
					{
						NpcInstance monster = npcSpawn.spawnOne();
						spawnedNpcs.add(monster);
						monster.doDie(activeChar);
						monster.decayMe();
					}

					for (NpcInstance spawnedNpc : spawnedNpcs) {
						if (spawnedNpc != null) {
							final Spawner spawner = spawnedNpc.getSpawn();
							if (spawner != null) {
								spawner.stopRespawn();
							}
							spawnedNpc.decayMe();
