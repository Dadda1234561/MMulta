		...
		switch(command)
		{
			...
			case admin_penalty:
				...
				else
				{
					...
					if(oId > 0)
					{
						String val = CharacterVariablesDAO.getInstance().getValue(oId, "penaltyChatCount");
						int oldCount = 0;
						if(val != null && !val.isEmpty())
							oldCount = Integer.parseInt(val);

						CharacterVariablesDAO.getInstance().insert(oId, new Variable("penaltyChatCount", String.valueOf(oldCount + count), -1));
					}
				}
