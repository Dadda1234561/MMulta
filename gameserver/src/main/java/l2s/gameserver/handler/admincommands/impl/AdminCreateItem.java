...
public class AdminCreateItem implements IAdminCommandHandler
{
	...
	public boolean useAdminCommand(Enum<?> comm, String[] wordList, String fullString, Player activeChar)
	{
		...
		switch(command)
		{
			...
			case admin_clearinv:
			{
				int[] leaveItemIds = new int[] {4037, 57, 48472};
				...
				try
				{
					...
					for (ItemInstance item : activeChar.getInventory().getItems())
					{
						...
						if (ArrayUtils.contains(leaveItemIds, item.getItemId()))
						{
							continue;
						}
						if (item.getExType().equals(ExItemType.SPIRITSHOT) || item.getExType().equals(ExItemType.SOULSHOT))
						{
							continue;
						}
						...
						synchronized (activeChar.getInventory())
						{
							...
							// iu.addRemovedItem(activeChar, item);
							cntr++;
						}
					}
					activeChar.sendMessage("Removed " + cntr + " items.");
			...
			case admin_create_item:
				...
				activeChar.sendPacket(new HtmlMessage(5).setFile("admin/itemcreation.htm"));
				break;
			case admin_create_online: {
				try {
					if(wordList.length < 2) {
						activeChar.sendMessage("USAGE: create_online id [count]");
						return false;
					}
					...
					for (Player player : GameObjectsStorage.getPlayers(false, false)) {
						if (player != null && player.isOnline()) {
							item = createItem(player, item_id, item_count);
						}
					}
					if(item == null) {
						activeChar.sendMessage("Undefined item id!");
