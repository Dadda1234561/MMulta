...
import l2s.gameserver.data.xml.parser.ChampionTemplateParser;
import l2s.gameserver.data.xml.parser.GearScoreParser;
import l2s.gameserver.data.xml.parser.UpMonsterTemplateParser;
...
public class AdminReload implements IAdminCommandHandler
{
	...
	private static enum Commands
	{
		...
		admin_reload_skills,
		admin_reload_champ_stats,
	 	admin_reload_upmob_stats,
		admin_reload_npc,
		admin_reload_spawn,
		admin_reload_fish,
		admin_reload_abuse,
		admin_reload_translit,
		admin_reload_transform,
		admin_reload_shops,
		admin_reload_static,
		admin_reload_pets,
		admin_reload_locale,
	...
	public boolean useAdminCommand(Enum<?> comm, String[] wordList, String fullString, final Player activeChar)
	{
			...
			public void sayYes()
			{
				switch(command)
				{
					...
					case admin_reload_skills:
					{
      				...
      				case admin_reload_upmob_stats:
					{
						UpMonsterTemplateParser.getInstance().reload();
						break;		
					}
					case admin_reload_champ_stats:
					{
						ChampionTemplateParser.getInstance().reload();
						break;
					}
		...
		switch(command)
		{
			...
			case admin_reload_transform:
			{
				TransformTemplateParser.getInstance().reload();
				break;
			}
			...
			case admin_reload_gearscore:
			{
				GearScoreHolder.getInstance().reload();
				break;
			}
		}
