...
public class AdminTeleport implements IAdminCommandHandler
{
	...
	public boolean useAdminCommand(Enum<?> comm, String[] wordList, String fullString, Player activeChar)
	{
		...
		switch(command)
		{
			...
			case admin_teleport:
				...
				int z_loc = ArrayUtils.valid(wordList, 3) != null && !ArrayUtils.valid(wordList, 3).isEmpty() ? Integer.parseInt(wordList[3].replace(".00", "")) : Config.MAP_MAX_Z;
	...
	private void teleportTo(Player activeChar, Player target, String Cords, int refId) {
		try {
			teleportTo(activeChar, target, Location.parseLoc(Cords.replaceAll("\\.00", "")), refId);
		} catch(IllegalArgumentException e) {
