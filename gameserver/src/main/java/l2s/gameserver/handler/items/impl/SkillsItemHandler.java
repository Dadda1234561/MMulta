...
public class SkillsItemHandler extends DefaultItemHandler
{
	...
	public void attachSkill(ItemTemplate itemTemplate, Skill skill) {
		...
		{
			if(haveNoAltSkill)
				LOGGER.warn(String.format("Item ID[%d] already has a \"no-alt\" skill: ID[%d] LEVEL[%d] SUB_LEVEL[%d] that can lead to malfunction of item!", itemTemplate.getItemId(), skill.getId(), SkillUtils.getSkillLevelFromMask(skill.getLevel()), SkillUtils.getSubSkillLevelFromMask(skill.getLevel())));
		}
