...
public class Party implements PlayerGroup
{
	...
	public void distributeXpAndSp(double xpReward, double spReward, Collection<Player> rewardedMembers, Creature lastAttacker, MonsterInstance monster)
	{
		...
		for(Player member : rewardedMembers)
		{
			if(!monster.isInRangeZ(member, Config.ALT_PARTY_DISTRIBUTION_RANGE))
				continue;
			if(member.getLevel() <= (partyLevel - (maxDistributionLevelDIff + member.getPlayer().getStat().calc(Stats.DIF_LEVEL_FOR_PENALTY))))
				continue;
			if(monster.getLevel() < member.getLevel() && monster.isMonster())
		...
		for(Player member : mtr)
		{
			int levelMemberForPenalty = member.getLevel();

			if(member.getPlayer().getKnownSkill(99003) != null && member.getLevel() > monster.getLevel())
				levelMemberForPenalty = Math.min(monster.getLevel(), member.getPlayer().getLevel() - (int) member.getPlayer().getStat().calc(Stats.DIF_LEVEL_FOR_PENALTY));

			double lvlPenalty = 1;
			lvlPenalty = Experience.penaltyModifier(monster.calculateLevelDiffForDrop(levelMemberForPenalty), 9);

			int lvlDiff = partyLevel - member.getLevel();
			lvlDiff = Math.max(0, Math.min(lvlDiff, maxDistributionLevelDIff));
			lvlPenalty *= (Config.ALT_PARTY_LVL_DIFF_PENALTY[lvlDiff] + member.getPlayer().getStat().calc(Stats.DIF_LEVEL_FOR_PENALTY)) / 100.;
