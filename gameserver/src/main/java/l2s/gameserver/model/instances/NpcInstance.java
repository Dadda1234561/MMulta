...
public class NpcInstance extends Creature
{
	...
	private ScheduledFuture<?> _minionSpawnTask;
	...
	public NpcInstance(int objectId, NpcTemplate template, MultiValueSet<String> set)
	{
		...
		/*if ((isMonster() *//*|| isBotTrap()*//*) && Config.SERVER_SIDE_NPC_TITLE_LVL_AGR) {
			String agresive = new CustomMessage("TitleLvLAgr").addNumber(getLevel()).toString();
			String none = new CustomMessage("TitleLvL").addNumber(getLevel()).toString();
			customTitle = isAggressive() ? agresive : none;
		}*/
		if (isMonster()){
			String agresive = "  Уровень: " + getLevel() + "*";
			String none = "  Уровень: " + getLevel();
			customTitle = isAggressive() ? agresive : none;
		}
	...
	protected void onDeath(Creature killer)
	{
		...
		if(hasMinions())
		{
			if (_minionSpawnTask != null)
			{
				_minionSpawnTask.cancel(false);
	...
	protected void onSpawn()
	{
		...
		if(hasMinions())
		{
			...
			_minionSpawnTask = ThreadPoolManager.getInstance().schedule(() -> getMinionList().spawnMinions(), 1500L); // DS: AI мастера должен запуститься раньше спавна миньонов.
		}
	...
	{
		final double baseExp = getStat().calc(Stats.EXP_RATE_MULTIPLIER, getTemplate().rewardExp, null, null);
		return (long) (baseExp * NpcUtils.getMonsterStatMod(Stats.EXP_RATE_MULTIPLIER, this));
	}
	...
	public void startRestoreTask(int upMonsterLvl, long delay)
	{
		stopRestoreTask();
		_restoreTask = ThreadPoolManager.getInstance().schedule(() ->
		{
			stopAbnormalEffect(AbnormalEffect.valueOf("AVE_STAR_X0" + getUpMonster() + "_EV"));
			removeUpMonsterStats(upMonsterLvl);
			removeUpMonsterDrop(upMonsterLvl);
			setCurrentHpMp(getMaxHp(), getMaxMp(), true);
			setUpMonster(0);
		}, delay);
	}

	private void removeUpMonsterStats(int upMonsterLvl) {
		ChampionTemplate template = UpMonsterTemplateHolder.getInstance().getTemplate(upMonsterLvl);
		if (template != null) {
			getStat().removeFuncsByOwner(template);
		}
	}

	private void removeUpMonsterDrop(int upMonsterLvl)
	{
		ChampionTemplate template = UpMonsterTemplateHolder.getInstance().getTemplate(upMonsterLvl);
		if (template != null)
		{
			RewardList toRemove = null;
			
			for (ChampionAdditionalDropData additionalDrop : template.getAdditionalDrop()) 
			{
				for (RewardList rewardList : getRewardLists()) 
				{
					...
					if (rewardList.getType().equals(RewardType.EVENT_GROUPED))
					{
						RewardGroup rewardGroup = rewardList.get(rewardList.size() - 1);
						if (rewardGroup != null)
						{
							for (RewardData rewardData : rewardGroup.getItems()) 
							{
								if (rewardData.getItemId() == additionalDrop.getId()) 
								{
									toRemove = rewardList;
									break;
								}
	...
	public void onAction(Player player, boolean shift)
	{
		...
		if (player.getAutoFarm().isFarmActivate() &&  /* player.getAutoShortCuts().autoSkillsActive() && */ player.isMageClass())
		{
			return;
		}
	...
	public List<L2GameServerPacket> addPacketList(Player forPlayer, Creature dropper)
	{
		...
		if (getAbnormalEffects().size() > 0)
			list.add(new NpcInfoAbnormalVisualEffect(this));
	...
	{
		return getStat().calc(Stats.DROP_COUNT_MODIFIER, 1.0d) * player.getDropCountMod();
	}

	public double getSpoilChanceMod(Player player) {
		return getStat().calc(Stats.SPOIL_CHANCE_MODIFIER, 1.0d) * player.getSpoilChanceMod();
	}

	public double getSpoilCountMod(Player player) {
		return getStat().calc(Stats.SPOIL_COUNT_MODIFIER, 1.0d) * player.getSpoilCountMod();
	}
