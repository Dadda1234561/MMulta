...
public class ItemInfo
{
	...
	private int _reuseTimeLeft;

	private boolean _isBlocked;
	private boolean _isBlessed;

	private Map<Integer, Ensoul> _normalEnsouls = Collections.emptyMap();
	private Map<Integer, Ensoul> _specialEnsouls = Collections.emptyMap();
	...
	private int calcItemReuse(ItemInstance item, Player player)
	{
		int reuseTimeleft = 0;
		if(player != null)
		{
			...
			if(sts != null && sts.hasNotPassed())
			{
				...
				if (reuseTimeleft > 0)
					setReuseTimeLeft(reuseTimeleft);
			}
		}

		return reuseTimeleft;
	}


	public ItemInfo(ItemInstance item, Player player, boolean isBlocked)
	{
		this(item, isBlocked);
		setReuseTimeLeft(calcItemReuse(item, player));
	}
	...
	public ItemInfo(ItemInstance item, boolean isBlocked)
	{
		...
		setUseCnt(item.getUsedCnt());
		setNormalEnsouls(item.getNormalEnsoulsMap());
		setSpecialEnsouls(item.getSpecialEnsoulsMap());
	}
	...
	public void setReuseTimeLeft(int timeLeft)
	{
		_reuseTimeLeft = timeLeft;
	}
	...
	public void setUseCnt(int times)
	{
		_usedTimes = times;
	}
	...
	public Map<Integer, Ensoul> getNormalEnsouls()
	{
		return _normalEnsouls;
	}

	public void setNormalEnsouls(Map<Integer, Ensoul> ensouls)
	{
		_normalEnsouls = ensouls;
	}

	public Map<Integer, Ensoul> getSpecialEnsouls()
	{
		return _specialEnsouls;
	}

	public void setSpecialEnsouls(Map<Integer, Ensoul> ensouls)
	{
		_specialEnsouls = ensouls;
	}

	public int getReuseTimeLeft()
	{
		return _reuseTimeLeft;
	}
	...
	private void setBlessed(boolean blessed)
	{
		_isBlessed = blessed;
	}
}