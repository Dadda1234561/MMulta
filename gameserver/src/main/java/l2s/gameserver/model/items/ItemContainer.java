...
public abstract class ItemContainer
{
	...
	public boolean destroyShot(ItemInstance item, long count) {
		if(item == null)
			return false;
	...
	public void checkItems() {
		Map<Integer, String> items = ItemDeletionHolder.getInstance().getItems();
		for (ItemInstance item : getItems()) {
			...
			{
				LocalDateTime localDateTime = LocalDateTime.parse(items.get(itemId), DateTimeFormatter.ofPattern("yyyy/MM/dd HH:mm:ss"));
				long millis = localDateTime.atZone(ZoneId.systemDefault()).toInstant().toEpochMilli();
				
				if ((item.getItemId() == itemId) && (millis <= System.currentTimeMillis()))
				{
					destroyItem(item);
				}
			}
