...
import l2s.gameserver.network.l2.s2c.herobook.ExHeroBookEnchant;
...
public class HeroBookManager
{
    ...
    static
    {
        ...
        EXPERIENCE.put(50, new HeroBookLevelHolder(50, 7465, new ItemData(82356, 1), SkillHolder.getInstance().getSkill(36319, 1), 250000, true));
        ...
        EXPERIENCE.put(100, new HeroBookLevelHolder(100, 15000, new ItemData(82356, 1), SkillHolder.getInstance().getSkill(36319, 2), 500000, true));
        ...
        EXPERIENCE.put(150, new HeroBookLevelHolder(150, 22500, new ItemData(82356, 1), SkillHolder.getInstance().getSkill(36319, 3), 1000000, true));
        ...
        EXPERIENCE.put(200, new HeroBookLevelHolder(200, 30000, new ItemData(82356, 1), SkillHolder.getInstance().getSkill(36319, 4), 1500000, true));
    ...
    public boolean tryEnchant(Player player, Map<Integer, Long> requestedItems)
    {
        ...
        final Set<ItemInstance> inventoryItems = new HashSet<>();
        ...
        {
            final ItemInstance requestedItem = inv.getItemByObjectId(item);
            if ((requestedItem != null) && (requestedItem.getLocation() != ItemInstance.ItemLocation.VOID) && (requestedItem.getOwnerId() == player.getObjectId()) && isValidItem(requestedItem.getItemId()) && (requestedItem.getCount() >= requestedItems.get(item)))
            {
                inventoryItems.add(requestedItem);
                final int points = ITEMS.getOrDefault(requestedItem.getItemId(), Integer.MIN_VALUE);
        ...
        for (ItemInstance item : inventoryItems)
        {
            if (!player.getInventory().destroyItemByObjectId(item.getObjectId(), requestedItems.get(item.getObjectId())))
            {
                return false;
            }
        }

        if  (!player.getInventory().reduceAdena(adenaCommission))
        {
            return false;
        }
    ...
    public void tryIncreaseLevel(Player player)
    {
        ...
        else
        {
            ...
            if (Rnd.get(100) <= successChance)
            {
                ...
                {
                    levelHolder.getItems().forEach(itemHolder -> DifferentMethods.getPay(player, itemHolder.getId(), itemHolder.getCount(), true));
                }
    ...
    public void saveCurrentPlayerProgress(Player player, HeroBookInfoHolder holder)
    {
        ...
        player.setVar(PlayerVariables.HERO_BOOK_PROGRESS, holder.getCurrentLevel() + ";" + holder.getCurrentExp());
    }

    public HeroBookInfoHolder getCurrentPlayerProgress(Player player)
    {
        final String rawValue = player.getVar(PlayerVariables.HERO_BOOK_PROGRESS, null);
        ...
        final String[] splitValue = rawValue.split(";");
        final int currentLevel = Integer.parseInt(splitValue[0]);
        final int currentLevelExp = Integer.parseInt(splitValue[1]);
        final HeroBookInfoHolder holder = new HeroBookInfoHolder();
    ...
    {
        final HeroBookInfoHolder holder = player.getHeroBookProgress();
        final Skill currentEffect = SkillHolder.getInstance().getSkill(36318, holder.getCurrentLevel());
        for (HeroBookLevelHolder levelInfoHolder : EXPERIENCE.values())
        {
            if (levelInfoHolder.getLevel() > holder.getCurrentLevel())
            {
                continue;
            }

            if (levelInfoHolder.getSkills() == null)
            {
                continue;
            }

            for (Skill skill : levelInfoHolder.getSkills())
            {
                if (player.getSkillLevel(skill.getId()) < skill.getLevel())
                {
                    player.addSkill(SkillEntry.makeSkillEntry(SkillEntryType.NONE, skill), false);
                }
            }
        }
        ...
        {
            player.addSkill(SkillEntry.makeSkillEntry(SkillEntryType.NONE, currentEffect), false);
        }
