...
public class HennaList implements Iterable<Henna>
{
	...
	public boolean refreshStats(boolean send)
	{
		...
		for (Henna henna : values())
		{
			...
			for (TIntIntIterator iterator = template.getSkills().iterator(); iterator.hasNext();)
			{
				...
				SkillEntry skillEntry = SkillEntry.makeSkillEntry(SkillEntryType.NONE, iterator.key(),
						iterator.value());
				if (skillEntry == null)
					continue;
			...
			if (activeStep > 0)
			{
				...
				{
					SkillEntry skillEntry = SkillEntry.makeSkillEntry(SkillEntryType.NONE, potentialEffect.getSkillId(),
							Math.min(activeStep, potentialEffect.getMaxSkillLevel()));
					if (skillEntry != null)
					{
						SkillEntry tempSkillEntry = skills.get(skillEntry.getId());
						if (tempSkillEntry == null || tempSkillEntry.getLevel() < skillEntry.getLevel())
	...
	public synchronized void tryPotenEnchant(int slot)
	{
		...
		{
			usedPower = 0;
			owner.setVar(HIDDEN_POWER_STEP_VAR, step + 1,
					TimeUtils.DAILY_DATE_PATTERN.next(System.currentTimeMillis()));
		}
