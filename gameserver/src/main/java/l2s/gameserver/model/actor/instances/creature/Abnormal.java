...
public final class Abnormal implements Runnable, Comparable<Abnormal>
{
	...
	private final List<EffectHandler> _effects = new CopyOnWriteArrayList<>();
	...
	private final SkillChain _skillChain;
	...
	public Abnormal(Creature effector, Creature effected, Skill skill, EffectUseType useType, boolean saveable)
	{
		...
		int chainSkillId = _skill.getChainSkillId();
		if (chainSkillId > 0) {
			SkillEntry skillEntry = SkillEntry.makeSkillEntry(SkillEntryType.CHAIN, chainSkillId, _skill.getChainSkillLevel());
			if (skillEntry != null)
				skillChain = new SkillChain(this, skillEntry);
		}
		_skillChain = skillChain;
	}
	...
	{
		// TODO: [Bonux] Временная заглушка. Убрать после переписи движка эффектов.
	...
	private void onStart()
	{
		...
		SkillChain skillChain = getSkillChain();
		if(skillChain != null)
			skillChain.checkTargetOwners();
	...
	public boolean checkBlockedAbnormalType(AbnormalType abnormal) {
		for (int i = 0; i < _effects.size(); i++) {
			if (_effects.get(i).checkBlockedAbnormalType(this, getEffector(), getEffected(), abnormal)) {
				return true;
			}
		}
		return false;
	...
	{
		for (int i = 0; i < _effects.size(); i++) {
			if (_effects.get(i).checkDebuffImmunity(this, getEffector(), getEffected())) {
				return true;
			}
		}
		return false;
	...
	public boolean canReplaceAbnormal(Skill skill, double minLeftTime) {
		if (skill.isPassive() || !skill.hasEffects(EffectUseType.NORMAL) && skill.getSkillType() != SkillType.SUMMON) {
			return false;
		}
		...
		if (!AbnormalList.checkAbnormalType(getSkill(), skill)) {
			return true;
		}

		if (getAbnormalLvl() < skill.getAbnormalLvl()) {
			return true;
		}

		return getTimeLeft() <= minLeftTime;
	}
}