...
public class CreatureMovement
{
	...
	public boolean isFollowing(int creatureObjId) {
		if (_followTarget.get() == null || !_isFollow) {
			return false;
		}
		return _followTarget.get().getObjectId() == creatureObjId;
	}
	...
	public Location getIntersectionPoint(Creature target)
	{
		...
		double range = (double) target.getMoveSpeed() / 2; // расстояние, пройденное за 1 секунду, равно скорости. Берем половину.
	...
	public boolean updatePosition()
	{
		...
		try
		{
			...
			if(isFollow())
			{
				if(getFollowTarget() == null || getFollowTarget().isInvisible(_actor))
				{
					stopMove();
					return false;
				}
				if(_actor.isInRangeZ(getFollowTarget(), getMoveOffset()) && GeoEngine.canSeeTarget(_actor, getFollowTarget()))
				{
					OnArrivedAction onArrivedAction = _onArrivedAction;
					stopMove();
					if(onArrivedAction != null)
			...
			if(_actor.isPlayer()){
				...
				if(_actor.isRunning() && _actor.getAI().isPatroller())
					_moveTaskDoneDist += (now - _startMoveTime) * _previousSpeed / 350.;
				else{
					_moveTaskDoneDist += (now - _startMoveTime) * _previousSpeed / 1000.;
				}
			}
			...
			if(isFollow())
			{
				...
				Location followLoc = getFollowTarget().getLoc();
				if(_movingDestTempPos.getDistance3D(followLoc) != 0)
				{
					...
					if(_followCounter == 5)
					{
						if(buildPathTo(followLoc.x, followLoc.y, followLoc.z, 0, getFollowTarget(), _forestalling, !getFollowTarget().isDoor()) != null)
