...
public class BookMarkList
{
	...
	{
		this.owner = owner;
		bookMarks = new ArrayList<>(acapacity);
		capacity = acapacity;
	}
	...
	{
		return bookMarks.add(e);
	}

	public BookMark get(int slot)
	{
		return bookMarks.get((slot - 1));
	}

	public void remove(int slot)
	{
		bookMarks.remove((slot -1));
		owner.removeBookmarkFromShortCut(slot);
	}
	...
	public boolean tryTeleport(int slot)
	{
		...
		BookMark bookmark = bookMarks.get((slot -1));
	...
	public boolean add(Location loc, String aname, String aacronym, int aiconId, boolean takeFlag)
	{
		...
		add(new BookMark((bookMarks.size() + 1), loc, aiconId, aname, aacronym));
	...
	public synchronized void restore()
	{
		...
		try
		{
			...
			bookMarks.clear();
			while(rs.next())
				add(new BookMark((bookMarks.size() + 1), rs.getInt("x"), rs.getInt("y"), rs.getInt("z"), rs.getInt("icon"), rs.getString("name"), rs.getString("acronym")));
		}
