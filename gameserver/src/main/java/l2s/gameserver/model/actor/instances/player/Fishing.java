	...
	private class FishingTask implements Runnable
	{
		...
		public void run()
		{
			...
			if(_fish.getId() == -1)
			{
				...
				_owner.getListeners().onFishing(Collections.emptyList(), false);
			}
			else
			{
				...
				if(rewards != null)
				{
					...
					exp = (long) (exp * _rod.getRewardModifier() * Config.RATE_XP_BY_LVL[_owner.getLevel()] * _owner.getPremiumAccount().getFishingExpRate());
					sp = (long) (sp * _rod.getRewardModifier() *  Config.RATE_SP_BY_LVL[_owner.getLevel()] * _owner.getPremiumAccount().getFishingSpRate());

					_owner.addExpAndSp(exp, sp, 0, 0, false, false, false, false, true);
				}

				List<ItemData> fishRewards = new ArrayList<>();
				fishRewards.add(new ItemData(_fish.getId(), Config.RATE_FISH_DROP_COUNT * 1));
				fishRewards.forEach((r) -> ItemFunctions.addItem(_owner, r.getId(), r.getCount(), true));

				_owner.broadcastPacket(new ExUserInfoFishing(_owner));
				_owner.broadcastPacket(new ExFishingEndPacket(_owner, ExFishingEndPacket.WIN));
				_owner.getListeners().onFishing(fishRewards, true);
			}
