...
public class DailyMissionList
{
    ...
    public boolean complete(int missionId)
    {
        ...
        if (fameReward > 0)
            _owner.setFame(_owner.getFame() + fameReward, String.format("Daily Mission ID[%d] reward.", missionTemplate.getId()), true);

        Clan clan = _owner.getClan();
        if (clan != null) {
            int clanReputationReward = missionTemplate.getClanReputationReward();
            if (clanReputationReward > 0)
                clan.incReputation(clanReputationReward, true, String.format("Daily Mission ID[%d] reward.", missionTemplate.getId()));
        }
