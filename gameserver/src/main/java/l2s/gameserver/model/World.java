...
public class World
{
	...
	public static void getAroundNpc(List<NpcInstance> result, GameObject object, ILocation point, WorldRegion region, int reflect, int radius, int height) {
		...
		int ox = point != null ? point.getX() : object.getX();
		int oy = point != null ? point.getY() : object.getY();
		int oz = point != null ? point.getZ() : object.getZ();
		int sqrad = radius * radius;

		int minX = validX(region.getX() - 1);
		int maxX = validX(region.getX() + 1);
		int minY = validY(region.getY() - 1);
		int maxY = validY(region.getY() + 1);
		int minZ = validZ(region.getZ() - 1);
		int maxZ = validZ(region.getZ() + 1);

		for (int x = minX; x <= maxX; x++) {
			for (int y = minY; y <= maxY; y++) {
				for (int z = minZ; z <= maxZ; z++) {
					for (GameObject obj : getRegion(x, y, z)) {
						if (!obj.isNpc() || obj.getReflectionId() != reflect) {
							continue;
						}
						if (Math.abs(obj.getZ() - oz) > height) {
							continue;
						...
						int dx = Math.abs(obj.getX() - ox);
						int dy = Math.abs(obj.getY() - oy);
						if (dx > radius || dy > radius || dx * dx + dy * dy > sqrad) {
							continue;
						}
						result.add((NpcInstance) obj);
					}
	...
	public static List<Creature> getAroundPlayers(GameObject object, ILocation point, int radius, int height)
	{
		int oid = object.getObjectId();
		int rid = object.getReflectionId();
		if(radius == -1)
	...
	public static List<Player> getAroundObservers(GameObject object)
	{
		...
		int y1 = validY(currentRegion.getY() + 1);
		int z0 = validZ(currentRegion.getZ() - 1);
		int z1 = validZ(currentRegion.getZ() + 1);
		for(int x = validX(currentRegion.getX() - 1); x <= x1; x++)
			for(int y = y0; y <= y1; y++)
				for(int z = z0; z <= z1; z++)
					for(GameObject obj : getRegion(x, y, z))
	...
	public static void getAroundItems(List<ItemInstance> result, GameObject object, ILocation point, int radius, int height)
	{
		if(radius < 0)
			return;
