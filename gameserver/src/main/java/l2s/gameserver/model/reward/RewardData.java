...
public class RewardData implements Cloneable
{
	...
	public RewardData(int itemId, long min, long max, double chance, int minLevel, int maxLevel)
	{
			...
			if(chance > 0)
			{
				...
				if(rolledCount > 0)
				{
					...
					if(_item.isStackable())
					{
						...
						if(getMinDrop() >= getMaxDrop())
							t.count = Math.round((rolledCount * getMinDrop() * countMod)); // округляем к ближайшему
						else
							t.count = Math.round((rolledCount * Rnd.get(getMinDrop(), getMaxDrop()) * countMod)); // округляем к ближайшему
					}
					if(t.count > 0)
						return t;
				}
