...
public class GameObjectTasks
{
	...
	public static class PvPFlagTask implements Runnable
	{
		...
		public void run()
		{
			...
			boolean inZone = player.isInZone(Zone.ZoneType.pvp_flag);
			long diff = Math.abs(System.currentTimeMillis() - player.getLastPvPAttack());
			if(diff > Config.PVP_TIME && !inZone)
				player.stopPvPFlag();
			else if(diff > Config.PVP_TIME - 20000 && !inZone)
	...
	/** FiveMinTask */
	public static class FiveMinTask implements Runnable
	{
		private final HardReference<Player> _playerRef;

		public FiveMinTask(Player player)
		{
			_playerRef = player.getRef();
		}
		...
		public void run()
		{
			...
			if(player == null)
				return;
			RankManager.getInstance().onPlayerEnter(player);
		}
	}
