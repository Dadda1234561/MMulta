...
public class CursedWeapon
{
	...
	public boolean dropIt(NpcInstance attackable, Player killer, Player owner)
	{
		...
		SkillEntry skillEntry = SkillEntry.makeSkillEntry(SkillEntryType.NONE, _skillId, player.getSkillLevel(_skillId));
		if(skillEntry != null)
		{
	...
	private Collection<SkillEntry> getSkills(Player player)
	{
		...
		SkillEntry skillEntry = SkillEntry.makeSkillEntry(SkillEntryType.NONE, _skillId, level);
		List<SkillEntry> ret = new ArrayList<SkillEntry>();
		ret.add(skillEntry);
		for(AddedSkill s : skillEntry.getTemplate().getAddedSkills())
			ret.add(SkillEntry.makeSkillEntry(SkillEntryType.NONE, s.id, s.level));
		return ret;
	}
