...
public class OlympiadGameTask implements Runnable
{
	...
	public void run()
	{
		...
		try
		{
			...
			if(!_game.checkPlayersOnline() && _status != BattleStatus.ValidateWinner && _status != BattleStatus.Ending && _status != BattleStatus.RoundInteval && _status != BattleStatus.Round)
			{
			...
			switch(_status)
			{
				...
				case CountDown:
				{
					...
					_count--;
					if(_count <= 0)
						task = new OlympiadGameTask(_game, BattleStatus.StartComp, 10, 1000);
					else
				...
				case StartComp:
				{
					if(_count == 10)
					{
						_game.sendMatchInfo(100);
						//_game.deleteBuffers();
						_game.setState(2);
						_game.sendStartMatch();
					}
					// Wait 100 sec (Battle)
					_count--;
					...
					if(_count <= 0){
						_game.setState(1);
						task = new OlympiadGameTask(_game, BattleStatus.TieMatch, 0, 10000);
					}
					else
				...
				case TieMatch:
					if(_game.chekTieMatch())
						task = new OlympiadGameTask(_game, BattleStatus.RoundInteval, _count, 5000);
					else task = new OlympiadGameTask(_game, BattleStatus.Ending, _count, 5000);
					break;
				...
				case RoundInteval:
					...
					if(_game.getRound() < 4){
						task = new OlympiadGameTask(_game, BattleStatus.Round, 10, 15000);
					}
					else {
						task = new OlympiadGameTask(_game, BattleStatus.Ending, 0, 20000); // на всякий случай
					}
					break;
				...
				case Round:
					...
					if(_count <= 0){
						_game.sendMatchInfo(20);
						_game.setState(1);
						task = new OlympiadGameTask(_game, BattleStatus.TieMatch, 0, 10000);
					}
					else
						task = new OlympiadGameTask(_game, BattleStatus.Round, _count, 10000);
					break;
				case Ending:
				{
					_game.collapse();
