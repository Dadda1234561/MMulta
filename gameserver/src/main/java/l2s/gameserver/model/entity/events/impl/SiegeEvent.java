...
public abstract class SiegeEvent<R extends Residence, S extends SiegeClanObject> extends Event
{
	...
	public long getRelation(Player thisPlayer, Player targetPlayer, long result)
	{
		...
		{
			result |= RelationChangedPacket.RelationChangedType.PLEDGE_IN_CASTLE_SIEGE.getRelationState();

			SiegeClanObject targetSiegeClan = getSiegeClan(SiegeEvent.ATTACKERS, thisPlayer.getClan());
			if (targetSiegeClan != null)
				result |= RelationChangedPacket.RelationChangedType.ATTACKER_IN_CASTLE_SIEGE.getRelationState();
			...
			if (targetPlayer.containsEvent(this))
			{
				...
				{
					result |= RelationChangedPacket.RelationChangedType.COMRADE_IN_CASTLE_SIEGE.getRelationState();
					if (playerSiegeClan == targetSiegeClan)
						result |= RelationChangedPacket.RelationChangedType.PLEDGE_IN_CASTLE_SIEGE.getRelationState();
				}
				else
				{
					result |= RelationChangedPacket.RelationChangedType.ENEMY_IN_CASTLE_SIEGE.getRelationState();
				}
			}
		}
		return result;
	...
	public int getUserRelation(Player thisPlayer, int oldRelation)
	{
		...
		if(siegeClan != null)
			oldRelation |= UIPacket.USER_RELATION_ATTACKER_IN_SIEGE;
