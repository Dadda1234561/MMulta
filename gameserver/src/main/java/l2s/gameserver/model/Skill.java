...
public class Skill extends StatTemplate implements SkillInfo, Cloneable
{
	...
	private final int[] _affectLimitPVP;
	private final int[] _affectLimitPVE;
	...
	private final int _reuseDelayPVP;
	private final int _reuseDelayPVE;
	...
	private final Integer _restore_time;
	...
	private final int _chainSkillId;
	private final int _chainSkillLevel;
	private final boolean _isPublicChain;
	...
	public Skill(StatsSet set)
	{
		...
		_reuseDelayPVP = set.getInteger("reuseDelayPVP", _reuseDelay);
		_reuseDelayPVE = set.getInteger("reuseDelayPVE", _reuseDelay);
		...
		_affectLimitPVP = set.getIntegerArray("affect_limitPVP", _affectLimit);
		_affectLimitPVE = set.getIntegerArray("affect_limitPVE", _affectLimit);
		...
		_restore_time = set.getInteger("restore_time", 15);
		...
		int[] chainSkill = set.getIntegerArray("chain_skill", new int[0], "-");
		_chainSkillId = chainSkill.length > 0 ? chainSkill[0] : 0;
		_chainSkillLevel = chainSkill.length > 1 ? chainSkill[1] : 1;
		_isPublicChain = set.getBool("is_public_chain", false);
	...
	public SystemMsg checkTarget(SkillEntry skillEntry, Creature activeChar, Creature target, Creature aimingTarget, boolean forceUse, boolean first, boolean trigger)
	{
		...
		if(skillEntry != null && skillEntry.isAltUse() || _targetType == SkillTargetType.TARGET_FEEDABLE_BEAST || _targetType == SkillTargetType.TARGET_UNLOCKABLE || _targetType == SkillTargetType.TARGET_CHEST)
			return null;
			...
			if(pcTarget != null)
			{
				...
				if(isDebuff())
				{
					if(pcTarget != player)
					{
						...
						if(player.isInCtF())
						{
							if(player.getTeam() != TeamType.NONE && pcTarget.getTeam() != TeamType.NONE && player.getTeam() == pcTarget.getTeam() && player != pcTarget) // Свою команду атаковать нельзя
								return SystemMsg.THAT_IS_AN_INCORRECT_TARGET;
						}
	...
	{
		boolean bothPlayers = (activeChar instanceof Playable) && (obj instanceof Playable);
		boolean bIsNotSelf = bothPlayers && activeChar.getObjectId() != obj.getObjectId();
		if (getEffectPoint() < 0 && isAoE() && bIsNotSelf && ((Playable) obj).getPvpFlag() != 0 && !obj.isInvisible(activeChar)) {
			return (Creature) obj;
		}
		...
		switch(_targetType)
		{
			...
			case TARGET_ONE_SUMMON:
				return target != null && target.isSummon() && activeChar.isMyServitor(target.getObjectId()) && target.isDead() == isCorpse() ? target : null;
			...
			case TARGET_PET:
				...
				return target != null && target.isPet() && target.isDead() == isCorpse() ? target : null;
			case TARGET_OWNER:
			...
			case TARGET_MENTEE:
				return target != null && target != activeChar && target.isDead() == isCorpse() && target.isPlayer() && target.getPlayer().getMenteeList().getMentor() == activeChar.getObjectId() ? target : null;
			case TARGET_MENTOR:
...
//				return target != null && target.isDead() == isCorpse() && !(target == activeChar && isDebuff()) && (!_isUndeadOnly || target.isUndead()) && activeChar.getPlayer().isInSameClan(target.getPlayer()) ? target : null;
				...
				if(cplayer != null && cplayer.isInOlympiadMode() && cptarget != null && cplayer.getOlympiadSide() == cptarget.getOlympiadSide() && cplayer.getOlympiadGame() == cptarget.getOlympiadGame() && target.isDead() == isCorpse() && !(target == activeChar && isDebuff()) && (!_isUndeadOnly || target.isUndead()))
					return target;
			...
			case TARGET_PARTY_ONE:
				...
				if(ptarget != null && ptarget == activeChar)
					return target;
				// olympiad party member or olympiad party member pet.
				if(player != null && player.isInOlympiadMode() && ptarget != null && player.getOlympiadSide() == ptarget.getOlympiadSide() && player.getOlympiadGame() == ptarget.getOlympiadGame() && target.isDead() == isCorpse() && !(target == activeChar && isDebuff()) && (!_isUndeadOnly || target.isUndead()))
					return target;
				// party member or party member pet.
	...
	public Set<Creature> getTargets(SkillEntry skillEntry, Creature activeChar, Creature aimingTarget, boolean forceUse)
	{
		...
		boolean bothPlayers = (activeChar instanceof Playable) && (aimingTarget instanceof Playable);
		boolean bIsNotSelf = bothPlayers && activeChar.getObjectId() != aimingTarget.getObjectId();
		if (getEffectPoint() < 0 && isAoE() && bIsNotSelf && (forceUse || aimingTarget.getPvpFlag() != 0) && !aimingTarget.isInvisible(activeChar)) {
		...
		{
			targets = new HashSet<>(1);
			if(aimingTarget != null && !aimingTarget.isInvisible(activeChar))
				targets.add(aimingTarget);
			if(_addSelfTarget)
				...
				if(activeChar.isPlayer())
				{
					...
					if(playerArena != -1)
					{
						...
						for(Player actor : World.getAroundPlayers(activeChar, 250, 100))
						{
							...
							{
								if(!actor.isInvisible(activeChar))
									targets.add(actor);
							}
						}
					}
	...
	private void addTargetsToList(SkillEntry skillEntry, Set<Creature> targets, Creature aimingTarget, Creature activeChar, boolean forceUse)
	{
		if(_targetType == SkillTargetType.TARGET_FAN || _targetType == SkillTargetType.TARGET_FAN_PB)
		{
			...
			if (aimingTarget.isPlayer())
				affectLimit = getAffectLimitPVP();
			if (aimingTarget.isMonster())
				affectLimit = getAffectLimitPVE();
		...
		else
		{
			...
			int pvpLimit = getAffectLimitPVP();
			int pveLimit = getAffectLimitPVE();
			...
			for(Creature target : arround)
			{
				if(!isLimitSet) {
					if (target.isPlayer()) {
						affectLimit = pvpLimit;
					} else if (target.isMonster()) {
						affectLimit = pveLimit;
					}
					isLimitSet = true;
				}
		...
		*/
		...
		if(effector != effected)
		{
			if(useType == EffectUseType.NORMAL)
			{
				...
				if(!isDebuff() && (effector.isCursedWeaponEquipped() || effected.isCursedWeaponEquipped()))
					return false;
			}
		}
		...
		for(Creature target : targets)
		{
			...
			else if(abnormalTimeModifier > 1.0)
			{
				...
				// На оффе скилл мастери проверяется для каждой цели индивидуально
				if(useType == EffectUseType.NORMAL && Formulas.calcSkillMastery(effector, this, true))
					duration *= 2;
			}
	...
	public final long getReuseDelayPvP()
	{
		return _reuseDelayPVP;
	}

	public final long getReuseDelayPvE()
	{
		return _reuseDelayPVE;
	}
	...
	public final int getAffectLimitPVP()
	{
		if(_affectLimitPVP[0] > 0 || _affectLimitPVP[1] > 0)
			return _affectLimitPVP[0] + (_affectLimitPVP[1] > 0 ? Rnd.get(_affectLimitPVP[1]) : 0);
		return 0;
	}

	public final int getAffectLimitPVE()
	{
		if(_affectLimitPVE[0] > 0 || _affectLimitPVE[1] > 0)
			return _affectLimitPVE[0] + (_affectLimitPVE[1] > 0 ? Rnd.get(_affectLimitPVE[1]) : 0);
		return 0;
	}
	...
	{
		return _isCancelable && _isSelfDispellable && !hasEffect(EffectUseType.NORMAL, "Transformation") && !isToggle() && !isVitalityLimited() && !isAura();
	}
		...
		if(_isUseSS == Ternary.DEFAULT)
		{
			...
			if(isPhysic())
			{
				if(getSkillType() == SkillType.CHARGE)
					return true;
				else if(getSkillType() == SkillType.CHARGE_SOUL)
					return true;
	...
	{
		return String.format("%s[id=%d, lvl=%d, sub_level=%d, hash_code=%d]", _name, _id, SkillUtils.getSkillLevelFromMask(_level), SkillUtils.getSubSkillLevelFromMask(_level), hashCode());
	}
	...
	**/
	...
	public SkillEntry getElementalSkill(SkillEntry skillEntry, Player player)
	{
		...
		SkillEntry elementalSkillEntry = SkillEntry.makeSkillEntry(SkillEntryType.NONE, elementalSkillId, getLevel());
	...
	**/
	...
	public int getChainSkillLevel()
	{
		return _chainSkillLevel;
	}

	public boolean isPublicChain()
	{
		return _isPublicChain;
	}
