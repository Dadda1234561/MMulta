...
public abstract class Servitor extends Playable
{
	...
	protected void onDelete()
	{
		...
		if(isSummon())
		{
			...
			Collections.reverse(summons);// Посылаем задом наперед, чтобы не менялся порядок.
	...
	public SkillEntry getAdditionalSSEffect(boolean spiritshot, boolean blessed)
	{
		...
		{
			// Ruby's
			if(ItemFunctions.checkIsEquipped(getPlayer(), -1, 48774, 0)) // Greater Ruby - Lv. 5
				return SkillEntry.makeSkillEntry(SkillEntryType.CUNSUMABLE_ITEM, 18716, 1);
			if(ItemFunctions.checkIsEquipped(getPlayer(), -1, 48773, 0)) // Greater Ruby - Lv. 4
				return SkillEntry.makeSkillEntry(SkillEntryType.CUNSUMABLE_ITEM, 18716, 1);
			if(ItemFunctions.checkIsEquipped(getPlayer(), -1, 48772, 0)) // Greater Ruby - Lv. 3
				return SkillEntry.makeSkillEntry(SkillEntryType.CUNSUMABLE_ITEM, 18716, 1);
			if(ItemFunctions.checkIsEquipped(getPlayer(), -1, 48771, 0)) // Greater Ruby - Lv. 2
				return SkillEntry.makeSkillEntry(SkillEntryType.CUNSUMABLE_ITEM, 18716, 1);
			if(ItemFunctions.checkIsEquipped(getPlayer(), -1, 47688, 0)) // Greater Ruby - Lv. 1
				return SkillEntry.makeSkillEntry(SkillEntryType.CUNSUMABLE_ITEM, 18716, 1);
