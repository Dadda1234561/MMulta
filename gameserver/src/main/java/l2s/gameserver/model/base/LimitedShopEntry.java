...
public class LimitedShopEntry
{
	...
	public LimitedShopIngredient getIngredient(int index)
	{
		return index >= _ingredients.size() ? null : _ingredients.get(index);
	}

	public LimitedShopProduction getProduction(int index)
	{
		return index >= _production.size() ? null : _production.get(index);
	}
