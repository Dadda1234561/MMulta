...
public class StatFunctions
{
	...
	private static class FuncPAtkMul extends Func
	{
		...
		public void calc(Env env, StatModifierType modifierType)
		{
			...
			if (env.character.isPlayer()) {
				int extraStatDiff = (env.character.getSTR() - env.character.getTemplate().getBaseSTR());
				env.value *= ((1 + (Math.max(0, extraStatDiff) * Config.STAT_STR_MODIFIER))) * Config.PLAYER_P_ATK_MODIFIER;
			}
		}
	}
	...
	private static class FuncMAtkMul extends Func
	{
		...
		public void calc(Env env, StatModifierType modifierType)
		{
			...
			if (env.character.isPlayer()) {
				int extraStatDiff = (env.character.getINT() - env.character.getTemplate().getBaseINT());
				env.value *= ((1 + (Math.max(0, extraStatDiff) * Config.STAT_INT_MODIFIER))) * Config.PLAYER_M_ATK_MODIFIER;
			}
		}
	}
	...
	private static class FuncMDefMul extends Func
	{
		...
		public void calc(Env env, StatModifierType modifierType)
		{
			...
			if (env.character.isPlayer()) {
				int extraStatDiff = (env.character.getMEN() - env.character.getTemplate().getBaseMEN());
				env.value *= (1 + (Math.max(0, extraStatDiff) * Config.STAT_MEN_MODIFIER));
			}
		}
	}
	...
	private static class FuncMCriticalRate extends Func
	{
		static final FuncMCriticalRate func = new FuncMCriticalRate();

		private FuncMCriticalRate()
		{
			super(Stats.BASE_M_CRITICAL_RATE, 0x70, null);
		}

		@Override
		public void calc(Env env, StatModifierType modifierType)
		{
			env.value += (env.character.getWIT() * 5);
		}
	}
	...
	private static class FuncPAtkSpeedMul extends Func
	{
		...
		public void calc(Env env, StatModifierType modifierType)
		{
			...
			if (env.character.isPlayer()) {
				int extraStatDiff = (env.character.getDEX() - env.character.getTemplate().getBaseDEX());
				env.value *= (1 + (Math.max(0, extraStatDiff) * Config.STAT_DEX_MODIFIER)) / 2;
			}
		}
	}
	...
	private static class FuncMAtkSpeedMul extends Func
	{
		...
		public void calc(Env env, StatModifierType modifierType)
		{
			...
			if (env.character.isPlayer()) {
				int extraStatDiff = (env.character.getWIT() - env.character.getTemplate().getBaseWIT());
				env.value *= (1 + (Math.max(0, extraStatDiff) * Config.STAT_WIT_MODIFIER)) / 2;
			}
		}
	}
...
//
	...
	private static class FuncMaxHpMul extends Func
	{
		...
		public void calc(Env env, StatModifierType modifierType)
		{
			...
			if (env.character.isPlayer())
			{
				env.value *= (1 + (Math.max(0, (env.character.getCON() - env.character.getTemplate().getBaseCON())) * Config.STAT_CON_MODIFIER));
			}
		}
	}
	...
	private static class FuncRunSpdLimit extends Func
	{
		...
		public void calc(Env env, StatModifierType modifierType)
		{
			if(Config.LIM_MOVE > 0)
				env.value = Math.min(env.character.getStat().calc(Stats.SPEED_LIMIT, Config.LIM_MOVE), env.value);
		}
	}
	...
	private static class FuncMReuseLimit extends Func
	{
		static final Func func = new FuncMReuseLimit();

		public FuncMReuseLimit()
		{
			super(Stats.MAGIC_REUSE_RATE, 0x100, null);
		}

		@Override
		public void calc(Env env, StatModifierType modifierType)
		{
			// снимать максимум 80% от отката
			if (env.skill != null) {
				env.value = Math.max((env.skill.getReuseDelay() * 0.20), env.value);
			}
		}
	}

	private static class FuncPReuseLimit extends Func
	{
		static final Func func = new FuncPReuseLimit();

		public FuncPReuseLimit()
		{
			super(Stats.PHYSIC_REUSE_RATE, 0x100, null);
		}
	...
	private static class FuncMusicReuseLimit extends Func
	{
		static final Func func = new FuncMusicReuseLimit();

		public FuncMusicReuseLimit()
		{
			super(Stats.MUSIC_REUSE_RATE, 0x100, null);
		}
	...
	private static class FuncMCritLimit extends Func
	{
		...
		public void calc(Env env, StatModifierType modifierType)
		{
			if(Config.LIM_MCRIT > 0)
				env.value = Math.min(env.character.getStat().calc(Stats.P_CRIT_RATE_LIMIT, Config.LIM_MCRIT), env.value);
		}
	}
	...
	public static void addPredefinedFuncs(Creature cha)
	{
		...
		if(cha.isPlayer())
		{
			...
			//cha.getStat().addFuncs(FuncHennaLUC.func);
			//cha.getStat().addFuncs(FuncHennaCHA.func);
			...
			cha.getStat().addFuncs(FuncMReuseLimit.func);
			cha.getStat().addFuncs(FuncPReuseLimit.func);
			cha.getStat().addFuncs(FuncMusicReuseLimit.func);
		...
		cha.getStat().addFuncs(FuncMCriticalRate.func);
