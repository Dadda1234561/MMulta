...
public final class TradeHelper
{
	...
	public static int restoreOfflineBuffers() throws Exception
	{
		...
		try
		{
			...
			statement = con.prepareStatement("SELECT c1.obj_id obj_id, c1.value expire_time, c5.value itemId, c2.value price, c3.value skills, c4.value title FROM character_variables AS c1 INNER JOIN character_variables AS c2 ON c1.obj_id = c2.obj_id AND c2.name = 'offlinebuff_price' INNER JOIN character_variables AS c5 on c1.obj_id = c5.obj_id AND c5.name = 'offlinebuff_itemid' INNER JOIN character_variables AS c3 ON c1.obj_id = c3.obj_id AND c3.name = 'offlinebuff_skills' LEFT JOIN character_variables AS c4 ON c1.obj_id = c4.obj_id AND c4.name = 'offlinebuff_title' WHERE c1.name = 'offlinebuff'");
			rset = statement.executeQuery();
			...
			while(rset.next())
			{
				...
				BufferData buffer = new BufferData(p, title, itemId, price, null);
