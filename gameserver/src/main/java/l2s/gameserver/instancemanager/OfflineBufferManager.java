...
public class OfflineBufferManager
{
	public static class BufferData
	{
		...
		public BufferData(Player player, String title, int itemId, long price, List<SkillEntry> buffs)
		{
			_owner = player;
			_saleTitle = title;
			_itemId = itemId;
	...
	public void processBypass(Player player, String command)
	{
		...
		if(cmd.equalsIgnoreCase("create"))
		{
			...
			long writedPrice;
			try {
				itemId = st.nextToken().trim().equals("MultCoin") ? 4037 : 57;
			...
			final BufferData buffer = new BufferData(player, title, itemId, price, null);
		...
		else if(cmd.equalsIgnoreCase("purchasebuff"))
		{
			...
			final int buffPriceId = buffer.getBuffItemId();
			...
			switch (buffPriceId)
			{
				...
				case 4037:
				{
					if(buffPrice > 0 && (player.getInventory().getCountOf(4037) < buffPrice) || !DifferentMethods.getPay(player, buffPriceId, buffPrice, true))
					{
						player.sendMessage(new CustomMessage("l2s.gameserver.instancemanager.OfflineBufferManager.purchasebuff.6"));
						return;
					}
			...
			if(buffPrice > 0 && buffPriceId == 57)
				caster.addAdena(buffPrice, true);
			else if (buffPrice > 0 && buffPriceId == 4037)
				DifferentMethods.addItem(caster, 4037, buffPrice);
	...
	private void showStoreWindow(Player player, BufferData buffer, int page)
	{
		...
		html = html.replace("<?buff_priceName?>", DifferentMethods.getItemName(buffer.getBuffItemId()));
	...
	public synchronized void storeBufferData(Player trader)
	{
		...
		if(buffer != null)
		{
			...
			trader.setVar("offlinebuff_itemid", buffer.getBuffItemId());
