...
public class DefaultAI extends NpcAI
{
	...
	public final int MAX_HATE_RANGE = 2600;
	...
	protected boolean thinkActive()
	{
		...
		if(now - _checkAggroTimestamp > Config.AGGRO_CHECK_INTERVAL)
		{
			...
			boolean aggressive = !actor.isChampion() && Rnd.chance(actor.getParameter("SelfAggressive", isAggressive() ? 100 : 0));
	...
	protected void onEvtSpawn()
	{
		...
		_UDFlag = UD_NONE;
		/*if(Config.SHOW_PRINT && getActor().isMonster())
			System.out.println("onEvtSpawn ((MonsterInstance) getActor()).getChampion() = " + ((MonsterInstance) getActor()).getChampion());*/
		if(getActor().isMonster()){
			if(getActor().getReflection().getId() == -1006){
				for(RewardList rewardList : getActor().getRewardLists()) {
					getActor().getRewardLists().remove(rewardList);
				}
			}
		}
	...
	protected void onEvtDead(Creature killer)
	{
		...
		if(killer != null && killer.isPlayable() && actor.isMonster())
		{
			final int diffPlayerTarget = Math.abs(killer.getLevel() - actor.getLevel());
			if(Rnd.get(100) <= 70)
			{
				if(diffPlayerTarget < 15 && actor.getAbnormalEffects().contains(AbnormalEffect.CHAMPIONS_AURAS_NEW_1)){
					actor.dropItem(killer.getPlayer(), 70004, Rnd.get(2, 2));
				} else if(diffPlayerTarget < 15 && actor.getAbnormalEffects().contains(AbnormalEffect.CHAMPIONS_AURAS_NEW_2)){
					actor.dropItem(killer.getPlayer(), 70004, Rnd.get(4, 4));
				} else if(diffPlayerTarget < 15 && actor.getAbnormalEffects().contains(AbnormalEffect.CHAMPIONS_AURAS_NEW_3)){
					actor.dropItem(killer.getPlayer(), 70004, Rnd.get(6, 6));
				} else if(diffPlayerTarget < 15 && actor.getAbnormalEffects().contains(AbnormalEffect.CHAMPIONS_AURAS_NEW_4)){
					actor.dropItem(killer.getPlayer(), 70004, Rnd.get(8, 8));
				} else if(diffPlayerTarget < 15 && actor.getAbnormalEffects().contains(AbnormalEffect.CHAMPIONS_AURAS_NEW_5)){
					actor.dropItem(killer.getPlayer(), 70004, Rnd.get(10, 10));
				}
			}
			// Astatin
			if(actor.getReflection().getId() == -1006){
				actor.dropItem(killer.getPlayer(), 9912, Rnd.get(15, 30));
			}
	...
	protected void onEvtAttacked(Creature attacker, Skill skill, int damage)
	{
		...
		// DM auto-spoil
		if(actor.isMonster() && player != null && player.getKnownSkill(70099) != null){
			MonsterInstance targetSpoil = (MonsterInstance) getActor();
			if(targetSpoil != null && !targetSpoil.isSpoiled()){
				targetSpoil.setSpoiled(player.getPlayer());
			}
		}
	}
