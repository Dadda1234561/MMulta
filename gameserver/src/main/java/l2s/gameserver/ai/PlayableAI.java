...
{
	private final AtomicInteger thinking = new AtomicInteger(0); // to prevent recursive thinking
	...
	public boolean setNextIntention()
	{
		...
		// Если маг класс - не ставить следующий интеншн на атакую..
		if (_actor.isPlayer() && _actor.isMageClass() && AINextAction.ATTACK.equals(nextAction))
		{
	...
	protected void thinkFollow()
	{
		...
		Creature target = _intention_arg0 instanceof GameObject ? (Creature) _intention_arg0 : null;
	...
	protected void thinkAttack(boolean arrived)
	{
		...
		{
			if (player.getAutoFarm().isFarmActivate())
		...
		System.out.println("clientStopMoving()");
		//clientStopMoving();
		actor.doAttack(attack_target)*/;
	}
	...
	protected boolean thinkCast(boolean arrived)
	{
		...
		if(!noRangeSkill)
		{
			...
			boolean canSee = isGroundSkill || skill.getSkillType() == SkillType.TAKECASTLE || skill.getSkillType() == SkillType.TAKEFORTRESS || GeoEngine.canSeeTarget(actor, target);
			if(!canSee)
			{
				if(actor.isPlayer())
				{
