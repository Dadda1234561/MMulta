...
{
	private static final int ENCHANT_DELAY = 500; // client has 600 ms delay
	...
	protected void runImpl()
	{
		...
		try
		{
			...
			ItemInstance catalyst = player.getSupportItem() == null ? null
					: inventory.getItemByObjectId(player.getSupportItem().getObjectId());
				...
				switch (enchantScroll.getType())
				{
					...
					case WEAPON:
						if (itemType == ItemTemplate.TYPE2_SHIELD_ARMOR || itemType == ItemTemplate.TYPE2_ACCESSORY
								|| item.getTemplate().isHairAccessory())
						{
							player.sendPacket(EnchantResult.CANCEL);
							player.sendPacket(SystemMsg.DOES_NOT_FIT_STRENGTHENING_CONDITIONS_OF_THE_SCROLL);
							player.sendActionFailed();
			...
			final EnchantVariation variation = EnchantItemHolder.getInstance()
					.getEnchantVariation(enchantScroll.getVariationId());
			if (variation == null)
			{
				player.sendActionFailed();
				_log.warn("RequestEnchantItem: Cannot find variation ID[" + enchantScroll.getVariationId()
			...
			if (enchantLevel == null)
			{
				player.sendActionFailed();
				_log.warn("RequestEnchantItem: Cannot find variation ID[" + enchantScroll.getVariationId()
						+ "] enchant level[" + (item.getEnchantLevel() + 1) + "] for enchant scroll ID["
			...
			if (Rnd.chance(chance))
			{
				...
				{
					player.broadcastPacket(new SystemMessage(SystemMessage.C1_HAS_SUCCESSFULY_ENCHANTED_A__S2_S3)
							.addName(player).addNumber(item.getEnchantLevel()).addItemName(item.getItemId()));
					player.broadcastPacket(new MagicSkillUse(player, player, SUCCESS_VISUAL_EFF_ID, 1, 500, 1500));
//					player.broadcastPacket(
//							new ExItemAnnounce(player.getName(), item.getItemId(), item.getEnchantLevel()));
				}
			...
			else
			{
				...
				switch (resultType)
				{
					...
					case DROP_ENCHANT:
						...
						final boolean isBlessedReset = enchantDropCount == Integer.MAX_VALUE;
						...
						if (isBlessedReset) {
							player.sendPacket(new EnchantResult(10 , 0, 0, 0));
						} else if (enchantDropCount > 0) {
							player.sendPacket(new EnchantResult(6, item.getItemId(), 1, item.getEnchantLevel()));
						}
						break;
					case NOTHING:
						player.sendPacket(new EnchantResult(8, item.getItemId(), 1, item.getEnchantLevel()));
						break;
				}
