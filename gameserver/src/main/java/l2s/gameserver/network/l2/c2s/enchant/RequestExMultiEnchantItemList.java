        ...
        try
        {
            ...
            for (int slotCounter = 0; slotCounter < slots.length; slotCounter++)
            {
                ...
                if (Rnd.chance(chance))
                {
                    ...
                    player.sendPacket(
                            new EnchantResult(0, 0, 0, enchantItem.getEnchantLevel(), enchantItem.getEnchantLevel()));
...
//                        player.broadcastPacket(new ExItemAnnounce(player.getName(), enchantItem.getItemId(), enchantItem.getEnchantLevel()));
                    }
                ...
                else
                {
                    ...
                    switch (resultType)
                    {
                        ...
                        case DROP_ENCHANT:
                            ...
                            player.sendPacket(new EnchantResult(1, enchantItem.getItemId(), 1, enchantItem.getEnchantLevel()));
                            _failureReward.put(_failureReward.size() + 1, new ItemData(enchantItem.getItemId(), 1));
                            _result.put(i, "FAIL");
                            break;
                        case NOTHING:
                            _result.put(i, "FAIL");
            ...
            for (int slotCounter = 0; slotCounter < slots.length; slotCounter++)
            {
                ...
                if (_result.get(i) == null) {
                    _log.info("[" + getClass().getSimpleName() + ":362]: result for slot " + i + " is null! ScrollId = " + enchantScroll.getItemId() + " slotIterator = " + slotCounter);
                    player.sendPacket(new ExResultMultiEnchantItemList(player, true));
                    return;
                }
