...
public class RequestExTimeRestrictFieldUserEnter extends L2GameClientPacket {
    ...
    protected void runImpl() {
        ...
        TimeRestrictFieldInfo field = TimeRestrictFieldHolder.getInstance().getFields().get(_zoneId);
		if (field != null) {
			int remainingTime = field.getRemainingTime(activeChar);
			if (remainingTime <= 0) {
				activeChar.sendMessage("No more time remaining...");
				return;
			}
		}
