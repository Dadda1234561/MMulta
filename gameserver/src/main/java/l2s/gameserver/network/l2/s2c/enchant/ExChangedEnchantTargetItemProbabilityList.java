...
{
    private static final Logger LOGGER = Logger.getLogger(ExChangedEnchantTargetItemProbabilityList.class.getName());
    ...
    private int getBaseRate(Player player, int i)
    {
        ...
        if (!_isMultiEnchant)
        {
            ...
            if (player.getEnchantScroll() == null) {
                LOGGER.info("Couldn't find player enchant scroll for player " + _player.getName());
                return -1;
            }
            final EnchantVariation variation = EnchantItemHolder.getInstance().getEnchantVariation(enchantScroll.getVariationId());
            if (variation == null) {
                LOGGER.info("Couldn't find variation ID=" + enchantScroll.getVariationId() + " for scroll " + enchantScroll.getItemId());
                return -1;
            }
