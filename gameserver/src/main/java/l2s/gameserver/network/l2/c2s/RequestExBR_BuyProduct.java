	...
	protected void runImpl()
	{
		...
		try
		{
			...
			else if(product.getPointsType() == ProductPointsType.ADENA)
			{
				...
				{
					activeChar.sendPacket(ExBR_BuyProductPacket.RESULT_NOT_ENOUGH_ADENA);
					return;
				}
			}
			else if(product.getPointsType() == ProductPointsType.FREE_COIN)
			{
				...
				{
					activeChar.sendPacket(ExBR_BuyProductPacket.RESULT_NOT_ENOUGH_FREE_COINS);
					return;
				}
			}
			else
			...
			activeChar.sendPacket(ExBR_NewIConCashBtnWnd.NO_UPDATES);	// TODO: Посылать при наличии новинок в Итем-молле 1, если нет, то 0.
