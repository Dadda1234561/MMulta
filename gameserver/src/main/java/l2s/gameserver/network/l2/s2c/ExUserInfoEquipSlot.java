...
public class ExUserInfoEquipSlot extends AbstractMaskPacket<InventorySlot>
{
	...
	{
		_player = player;
		if (slot >= InventorySlot.values().length) {
			slot = InventorySlot.BROOCH_JEWEL.getSlot();
		}
	...
	protected final void writeImpl()
	{
		...
		for(InventorySlot slot : InventorySlot.VALUES)
		{
			...
			{
				int paperdollVisualId = inventory.getPaperdollVisualId(slot.getSlot());

				writeH(22); // size
				writeD(inventory.getPaperdollObjectId(slot.getSlot()));
				writeD(paperdollVisualId > 0 ? paperdollVisualId : inventory.getPaperdollItemId(slot.getSlot()));
				writeD(inventory.getPaperdollVariation1Id(slot.getSlot()));
				writeD(inventory.getPaperdollVariation2Id(slot.getSlot()));
				writeD(paperdollVisualId);
			}
		}
	}
}