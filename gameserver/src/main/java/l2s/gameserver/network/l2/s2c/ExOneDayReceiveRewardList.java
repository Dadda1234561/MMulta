...
public class ExOneDayReceiveRewardList extends L2GameServerPacket
{
	...
	{
		_dayRemainTime = (int) ((DAILY_REUSE_PATTERN.next(System.currentTimeMillis()) - System.currentTimeMillis())
				/ 1000);
		_weekRemainTime = (int) ((WEEKLY_REUSE_PATTERN.next(System.currentTimeMillis()) - System.currentTimeMillis())
				/ 1000);
		_monthRemainTime = (int) ((MONTHLY_REUSE_PATTERN.next(System.currentTimeMillis()) - System.currentTimeMillis())
				/ 1000);
		...
		for (DailyMissionTemplate missionTemplate : player.getDailyMissionList().getAvailableMissions())
		{
			...
			{
				DailyMissionTemplate completedMissionTemplate = DailyMissionsHolder.getInstance()
						.getMission(missionTemplate.getCompletedMission());
	...
	protected void writeImpl()
	{
		...
		//	player.sendPacket(new SayPacket2(player.getObjectId(), ChatType.ALL, "Daily Mission Pull: ", String.valueOf(mission)));
		//}
