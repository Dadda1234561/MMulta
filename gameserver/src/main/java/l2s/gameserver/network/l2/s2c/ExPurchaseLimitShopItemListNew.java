...
public class ExPurchaseLimitShopItemListNew extends L2GameServerPacket
{
	...
	protected final void writeImpl()
	{
		...
		for (int i = 0; i < _size; i++)
		{
			...
			writeD(_player.getVarInt(
					PlayerVariables.LIMIT_ITEM_REMAIN + "_" + product.getInfo().getInteger("product1Id"),
					product.getInfo().getInteger("dailyLimit"))); // nRemainItemAmount
			writeD(0); // nRemainSec
