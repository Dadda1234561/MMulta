...
public class RequestHennaEquip extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		 ...
		 * if(template == null) {
		 * player.sendPacket(SystemMsg.THE_SYMBOL_CANNOT_BE_DRAWN); return; }
		 *
		 * Henna henna = new Henna(template, (int) (System.currentTimeMillis() / 1000),
		 ...
		 * return; }
		 *
		 * if(template.getDyeLvl() > player.getLevel()) { player.sendPacket(SystemMsg.
		 ...
		 * if(countDye < template.getDrawCount()) { player.sendPacket(SystemMsg.
		 * YOU_CANNOT_RECEIVE_A_SYMBOL_BECAUSE_YOU_DONT_HAVE_ENOUGH_DYE); return; }
		 *
		 * long drawPrice = template.getDrawPrice(); if(drawPrice > 0 && drawPrice >
