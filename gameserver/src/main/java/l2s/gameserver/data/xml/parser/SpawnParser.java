	...
	protected void readData(Element rootElement) throws Exception
	{
		...
		for(Iterator<Element> spawnIterator = rootElement.elementIterator(); spawnIterator.hasNext();)
		{
			...
			else if(spawnElement.getName().equalsIgnoreCase("spawn"))
			{
				...
				String announceBeforeSpawn = spawnElement.attributeValue("announce_before_spawn", "");
				String announceType = spawnElement.attributeValue("announce_type", "DEFAULT");
				...
				if (!announceBeforeSpawn.isEmpty()) {
					for (String minute : announceBeforeSpawn.split(";")) {
						template.addPreAnnounceMinute(Integer.parseInt(minute));
					}
				}
				template.setAnnounceType(announceType);
