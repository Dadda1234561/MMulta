...
public final class NpcParser extends AbstractParser<NpcHolder>
{
	...
	protected void readData(Element rootElement) throws Exception
	{
		for(Iterator<Element> npcIterator = rootElement.elementIterator(); npcIterator.hasNext();)
		{
			...
			if(Config.DROP_GLOBAL_ITEMS) {
				if (Config.GLOBAL_DROP.isEmpty()) {
					Config.parseGlobalDropInfo();
				}
				...
				final List<RewardData> customEventDrop = Config.GLOBAL_DROP.getOrDefault(template.isInstanceOf(RaidBossInstance.class) ? "RAID" : "MONSTER", Collections.emptyList());
				for(RewardData drop : customEventDrop) {
					if (template.level >= drop.getMinLevel() && template.level <= drop.getMaxLevel()) {
						rewardGroup.addData(drop);
					}
				}
				if (!rewardGroup.getItems().isEmpty()) {
					rewardList.add(rewardGroup);
					template.addRewardList(rewardList);
				}
			}
			if(Config.DROP_IMPROVED_ITEMS) {
				if (Config.IMPROVED_DROP_1.isEmpty()) {
					Config.parseImprovedDropInfo(1);
				}
				if (Config.IMPROVED_DROP_2.isEmpty()) {
					Config.parseImprovedDropInfo(2);
				}
				if (Config.IMPROVED_DROP_3.isEmpty()) {
					Config.parseImprovedDropInfo(3);
				}
				if (Config.IMPROVED_DROP_4.isEmpty()) {
					Config.parseImprovedDropInfo(4);
				}
				if (Config.IMPROVED_DROP_5.isEmpty()) {
					Config.parseImprovedDropInfo(5);
				}
			}

			if(Config.DROP_IMPROVED_CHAMPION_ITEMS) {
				if (Config.IMPROVED_DROP_CHAMPION_1.isEmpty()) {
					Config.parseImprovedChampionDropInfo(1);
				}
				if (Config.IMPROVED_DROP_CHAMPION_2.isEmpty()) {
					Config.parseImprovedChampionDropInfo(2);
				}
				if (Config.IMPROVED_DROP_CHAMPION_3.isEmpty()) {
					Config.parseImprovedChampionDropInfo(3);
				}
				if (Config.IMPROVED_DROP_CHAMPION_4.isEmpty()) {
					Config.parseImprovedChampionDropInfo(4);
				}
				if (Config.IMPROVED_DROP_CHAMPION_5.isEmpty()) {
					Config.parseImprovedChampionDropInfo(5);
				}
			}
