...
public final class DailyMissionsParser extends AbstractParser<DailyMissionsHolder>
{
    ...
    protected void readData(Element rootElement) throws Exception
    {
        for (Iterator<Element> iterator = rootElement.elementIterator(); iterator.hasNext();)
        {
            ...
            int minRebirth = parseInt(element, "minRebirth", 1);
            int clanReputationReward = parseInt(element, "clan_reputation_reward", 0);
            int fameReward = parseInt(element, "fame_reward", 0);

            DailyMissionTemplate mission = new DailyMissionTemplate(id, handler, value, minLevel, maxLevel, completedMission, minRebirth, clanReputationReward, fameReward);
            ...
            for (Iterator<Element> rewardsIterator = element.elementIterator("rewards"); rewardsIterator.hasNext();)
            {
                ...
                TIntSet classIds = classes == null ? null
                        : new TIntHashSet(
                        StringArrayUtils.stringToIntArray(rewardsElement.attributeValue("classes"), ","));

                DailyRewardTemplate reward = new DailyRewardTemplate(classIds);

                for (Iterator<Element> rewardIterator = rewardsElement.elementIterator("reward"); rewardIterator
                        .hasNext();)
                {
