...
public class FestivalBMParser extends AbstractParser<FestivalBMHolder>
{
	...
	{
		return new File(Config.DATAPACK_ROOT, "data/festival_data/FestivalBMData.xml");
	}
	...
	protected void readData(Element rootElement) throws Exception
	{
		...
		for (Iterator<Element> iterator = rootElement.elementIterator("item"); iterator.hasNext();)
		{
			...
			long totalCount = parseLong(element, "totalCount", 1);
			double chance = parseDouble(element, "chance", 100.0d);
			int locationId = parseInt(element, "locationId");

			getHolder().addFestivalBMInfo(i, new FestivalBMTemplate(itemId, count, totalCount, chance, locationId));
			i++;
		}
	}
}
