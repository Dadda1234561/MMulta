	...
	protected MultiSellEntry parseEntry(Element n, int multiSellId)
	{
		...
		for(Iterator<Element> iterator = n.elementIterator(); iterator.hasNext();)
		{
			...
			if("ingredient".equalsIgnoreCase(d.getName()))
			{
				...
				int enchant = parseInt(d, "enchant", 0);
				entry.addIngredient(new MultiSellIngredient(id, count, 100, -1, -1, enchant));
			}
				...
				if(flagsArray != null)
				{
					for(String flag : flagsArray)
					{
						switch(flag)
						{
							...
							case "FLAG_NO_SHAPE_SHIFTING": flags |= ItemInstance.FLAG_NO_SHAPE_SHIFTING; break;
						}
					}
				}
