...
{
	private static final Pattern LINE_PATTERN = Pattern.compile("^([0-9]+)\\t([0-9]+)\\t([0-9]+)\\t(.*?)$");
				...
				try
				{
					...
					while(scanner.hasNextLine())
					{
						...
						if(m.find())
						{
							...
							int subLevel = Integer.parseInt(m.group(3));
							int levelMask = SkillUtils.getSkillLevelMask(level, subLevel);
							int hashCode = SkillHolder.getInstance().getHashCode(id, levelMask);
