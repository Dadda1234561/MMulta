	...
	public boolean useVoicedCommand(String command, Player activeChar, String target)
	{
		if(!target.isEmpty())
		{
			...
			try
			{
				...
				statement = con.prepareStatement("SELECT c.karma, c.sex, cs.default_class_id FROM characters AS c LEFT JOIN character_dualclasses AS cs ON c.obj_Id=cs.char_obj_id WHERE c.obj_Id=? AND cs.type=?");
				...
				if(rs.next())
				{
					...
					ClassId classId = ClassId.valueOf(rs.getInt("default_class_id"));
