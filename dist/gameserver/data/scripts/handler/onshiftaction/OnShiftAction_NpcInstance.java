...
public class OnShiftAction_NpcInstance extends ScriptOnShiftActionHandler<NpcInstance>
{
	...
	private boolean showMain(Player player, NpcInstance npc, boolean full)
	{
		...
		if(full)
		{
			...
			msg.replace("%xyz%", npc.getLoc().x + " " + npc.getLoc().y + " " + npc.getLoc().z + " " + npc.getLoc().h);
		...
		final int statModGroup = npc.isRaid() ? NpcUtils.getRaidLvlGroup(npc) : NpcUtils.getMonsterLvlGroup(npc);
		...
		msg.replace("<?stat_mod_group?>", String.valueOf(statModGroup));
