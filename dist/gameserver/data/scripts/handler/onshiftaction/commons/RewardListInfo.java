...
public abstract class RewardListInfo
{
	...
	public static void showInfo(Player player, NpcInstance npc, RewardType showType, int page)
	{
		...
		else
		{
			...
			List<RewardList> ratedCountRewardLists = new ArrayList<RewardList>();
		...
		if(show)
		{
			...
			for(RewardList list : rewardLists)
			{
				...
				for(RewardGroup g : list)
				{
					...
					{
						if(!d.getItem().isHerb() || groupType == RewardType.EVENT_GROUPED || groupType == RewardType.RATE_COUNT_GROUPED)
							items.add(d);
					}
					...
					else if (type == RewardType.RATE_COUNT_GROUPED)
					{
					...
					else if(type == RewardType.SWEEP)
					{
						...
						gpmod *= npc.getSpoilChanceMod(player);
						gcount = npc.getSpoilCountMod(player);
					}
					...
					double itemMaxChance = 0;
					if(type == RewardType.EVENT_GROUPED)
						itemMaxChance = (double) RewardList.MAX_CHANCE * Math.min(g.getItems().size(), Config.MAX_DROP_ITEMS_FROM_ONE_GROUP) / RewardList.MAX_CHANCE;
					else{
						itemMaxChance = (double) RewardList.MAX_CHANCE / g.getItems().size() * Math.min(g.getItems().size(), Config.MAX_DROP_ITEMS_FROM_ONE_GROUP) / RewardList.MAX_CHANCE;
					}
					...
					if(type == RewardType.EVENT_GROUPED)
						itemMaxChance += normalChancesSum;
					else{
			...
			for(int i = ((currentPage - 1) * MAX_ITEMS_ON_PAGE); i < Math.min(currentPage * MAX_ITEMS_ON_PAGE, infos.size()); i++)
			{
				...
				dropBlock = dropBlock.replace("<?drop_count?>", String.valueOf(info.minCount));
				dropBlock = dropBlock.replace("<?drop_min_count?>", String.valueOf(info.minCount));
				dropBlock = dropBlock.replace("<?drop_max_count?>", String.valueOf(Math.abs(info.maxCount)));
