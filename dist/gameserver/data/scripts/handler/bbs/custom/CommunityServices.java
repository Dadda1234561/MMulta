		...
		if("cbbsservices".equals(cmd))
		{
			...
			else if("cabinet".equals(cmd2))
			{
				if(st.hasMoreTokens())
				{
					...
					else if("aa".equals(cmd3))
					{
						IVoicedCommandHandler vch = VoicedCommandHandler.getInstance().getVoicedCommandHandler("aa");
						if(vch != null)
							vch.useVoicedCommand("aa", player, "");
						return;
					}
				...
				else
				{
					...
					html = html.replace("<?level?>", String.valueOf(player.getLevel()));
					...
					html = html.replace("<?rebirths?>", String.valueOf(player.getRebirthCount()));
					html = html.replace("<?noble?>", player.isNoble() ? (player.isLangRus() ? "Да" : "Yes") : (player.isLangRus() ? "Нет" : "No"));
					html = html.replace("<?clan_name?>", player.getClan() != null ? player.getClan().getName() : (player.isLangRus() ? "Нет" : "None"));
					html = html.replace("<?ally_name?>", player.getAlliance() != null ? player.getAlliance().getAllyName() : (player.isLangRus() ? "Нет" : "None"));
			...
			else if("changename".equals(cmd2))
			{
				...
				if("player".equals(cmd3))
				{
					...
					else
					{
						...
						String forTestAdminName = newPlayerName.toLowerCase();
						if(forTestAdminName.equals("admin") || forTestAdminName.equals("админ")) {
							if (player.getLanguage() == Language.RUSSIAN)
								player.sendScreenMessage("Запрещённое имя!");
							else
								player.sendScreenMessage("Forbidden name!");
							return;
				...
				else if("clan".equals(cmd3))
				{
					...
					else
					{
						...
						else if(clan.getName().equals(newClanName))
							content.append(tpls.get(6));
					...
					if(!st.hasMoreTokens()) {
						...
						for(String color : availableColors) {
							...
							if(player.getNameColor() != Integer.decode("0x" + bgrColor)) {
								if (count % 6 == 0) {
									if (count > 0) content.append("</tr>");
									content.append("<tr>");
								}
								content.append("<td>").append(colorBlock.replace("<?color?>", color)).append("</td>");
								count++;
							}
						}
				...
				else if("title".equals(cmd3))
				{
					...
					if(!st.hasMoreTokens())
					{
						...
						final String colorBlockTemplate = tpls.get(3).replace("<?player_name?>", player.getName());
						String[][] colorRows = new String[(int) Math.ceil((double) availableColors.length / 6)][6];
						int index = 0;
						for (int i = 0; i < colorRows.length; i++) {
							for (int j = 0; j < 6; j++) {
								if (index < availableColors.length) {
									...
									String bgrColor = color.substring(4, 6) + color.substring(2, 4) + color.substring(0, 2);
									if (player.getTitleColor() != Integer.decode("0x" + bgrColor) || color.equalsIgnoreCase("77FFFF")) {
										colorRows[i][j] = colorBlockTemplate.replace("<?color?>", color);
									} else {
										colorRows[i][j] = "";
									}
								} else {
						...
						content.append("<table>");
						for (String[] colorRow : colorRows) {
							content.append("<tr>");
							for (String colorCell : colorRow) {
								content.append("<td>").append(colorCell).append("</td>");
							}
							content.append("</tr>");
						}
						content.append("</table>");
			...
			else if("sex".equals(cmd2))
			{
				...
				else
				{
					...
					if("change".equals(cmd3))
					{
						...
						if(Race.ERTHEIA.equals(player.getVisualRace()) || player.getVar(Player.VISUAL_RACE, "").equals("DEATH_KNIGHT"))
						{
							player.sendMessage(new CustomMessage("common.NotAvailableClass").toString(player));
							return;
						}
						...
						else
						{
							player.changeSex();
							// fix hair style
							if (player.getSex().equals(Sex.MALE) && player.getHairStyle() > 4) {
								player.setHairStyle(4);
							}
							// fix hair color
							if (player.getRace().equals(Race.KAMAEL) || player.getRace().equals(Race.ERTHEIA) && player.getHairColor() > 3) {
			...
			else if("changeLevel".equals(cmd2))
			{
				final int feeItemId = BBSConfig.CHANGE_LEVEL_SERVICE_COST_ITEM_ID;
				final long feeItemCount = BBSConfig.CHANGE_LEVEL_SERVICE_COST_ITEM_COUNT;
				if(feeItemId == 0)
				{
					player.sendMessage(player.isLangRus() ? "Данный сервис отключен." : "This service disallowed.");
					player.sendPacket(ShowBoardPacket.CLOSE);
				...
				HtmTemplates tpls = HtmCache.getInstance().getTemplates("scripts/handler/bbs/pages/change_level.htm", player);
				html = tpls.get(0);
				...
				else
				{
					...
					if("change".equals(cmd3))
					{
						...
						{
							player.changePlayerLevel(player, 1);
							content.append(tpls.get(5).replace("<?player_name?>", player.getName()));
							player.broadcastUserInfo(true);
							player.broadcastPacket(new MagicSkillUse(player, player, 23128, 1, 1, 0));
							Log.add("Player " + player.getName() + " changed level to : " + player.getLevel(), "level.");
						}
					}
				}
				html = html.replace("<?content?>", content.toString());
			...
			else if("karma".equals(cmd2))
			{
			...
			else if ("visualrace".equals(cmd2)) {
				final int feeItemId = BBSConfig.CHANGE_RACE_SERVICE_COST_ITEM_ID;
				final long feeItemCount = BBSConfig.CHANGE_RACE_SERVICE_COST_ITEM_COUNT;
				if(feeItemId == 0)
				{
					player.sendMessage(player.isLangRus() ? "Данный сервис отключен." : "This service disallowed.");
					player.sendPacket(ShowBoardPacket.CLOSE);
				...
				HtmTemplates tpls = HtmCache.getInstance().getTemplates("scripts/handler/bbs/pages/change_race.htm", player);
				html = tpls.get(0);
				...
				if(!st.hasMoreTokens())
				{
					...
					content.append(tpls.get(3));
//					System.out.println(buildRaceTemplate(player, tpls));
//					content.append(buildRaceTemplate(player, tpls));
				}
				else
				{
					String cmd3 = st.nextToken();
					if(cmd3.startsWith("change")) {
						boolean isDK = false;
						String split = cmd3.split(":")[1];
						if(split == null)
						{
							return;
						}
						Race race;
						if(split.equals("DEATHKNIGHT"))
						{
							race = Race.HUMAN;
							isDK = true;
						}
						...
						{
							String oriSexStr = player.getVar(Player.ORIGINAL_SEX, null);
							Sex oriSex = oriSexStr == null ? player.getSex() : Sex.valueOf(oriSexStr);
							PlayerTemplate playerTemplate = PlayerTemplateHolder.getInstance().getPlayerTemplate(race, player.getClassId(), oriSex);
							content.append(tpls.get(5).replace("<?player_name?>", player.getName()));
							if(race.equals(Race.ERTHEIA)) {
								player.setVar(Player.ORIGINAL_SEX, player.getSex().name());
								PlayerTemplate ertheiaTemplate = PlayerTemplateHolder.getInstance().getPlayerTemplate(Race.ERTHEIA, ClassId.ERTHEIA_WIZARD, Sex.FEMALE);
								if(ertheiaTemplate != null) {
									player.setTemplate(ertheiaTemplate);
								}
							} else {
								player.unsetVar(Player.ORIGINAL_SEX);
								if(playerTemplate != null) {
									player.setTemplate(playerTemplate);
								}
							}

							player.setVisualRace(race);
							player.setVar(Player.VISUAL_RACE, isDK ? "DEATH_KNIGHT" : race.name());
							if (player.getTemplate() != null) {
								...
								switch (race) {
									case HUMAN:
										if (player.isMageClass()) {
											collisionRadius = player.getSex() == Sex.MALE ? 7.5 : 6.5;
											collisionHeight = player.getSex() == Sex.MALE ? 22.8 : 22.5;
										} else {
											collisionRadius = player.getSex() == Sex.MALE ? 9.0 : 8.0;
											collisionHeight = player.getSex() == Sex.MALE ? 23.0 : 23.5;
										}
										break;
									case ELF:
									case DARKELF:
										collisionRadius = 7.5;
										collisionHeight = player.getSex() == Sex.MALE ? 24.0 : player.isMageClass() ? 23.0 : 23.5;
										break;
									case ORC:
										if (player.isMageClass()) {
											collisionRadius = player.getSex() == Sex.MALE ? 7.0 : 8.0;
											collisionHeight = player.getSex() == Sex.MALE ? 27.5 : 25.5;
										} else {
											collisionRadius = player.getSex() == Sex.MALE ? 11.0 : 7.0;
											collisionHeight = player.getSex() == Sex.MALE ? 28.0 : 27.0;
										}
										break;
									case ERTHEIA:
										collisionRadius = 6.0;
									...
									case KAMAEL:
										if (player.isMageClass()) {
											collisionRadius = player.getSex() == Sex.MALE ? 8.0 : 7.0;
											collisionHeight = player.getSex() == Sex.MALE ? 25.2 : 22.6;
										} else {
											collisionRadius = player.getSex() == Sex.MALE ? 8.0 : 7.0;
											collisionHeight = player.getSex() == Sex.MALE ? 25.2 : 22.6;
										}
									...
									case DWARF:
										collisionRadius = 9.0;
										collisionHeight = player.isMageClass() ? 18.0 : player.getSex() == Sex.MALE ? 18.0 : 19.0;
										break;
								}

								player.getTemplate().setCollisionRadius(collisionRadius);
								player.getTemplate().setCollisionHeight(collisionHeight);
							}
							...
							player.broadcastPacket(new MagicSkillUse(player, player, 23128, 1, 1, 0));
							player.sendMessage("Поздравляем! Вы успешно сменили расу на " + race.getName(player) + "!");
							Log.add("Player " + player.getName() + " changed race to : " + player.getVisualRace(), "changerace");
						}
					}
	...
	private String buildRaceTemplate(Player player, HtmTemplates tpls) {
		String raceTpl = tpls.get(6);
		final StringBuilder raceList = new StringBuilder();
		raceList.append("<table><tr>");
		Race[] values = Race.values();
		for (int i = 0; i < values.length; i++) {
			Race race = values[i];
			raceTpl = raceTpl.replace("{race_name}", race.getName(player));
			raceTpl = raceTpl.replace("{race}", race.name());

			// skip current race
			if (race.equals(player.getRace())) {
				continue;
			}
			raceList.append("<td>").append(raceTpl).append("</td>");
			if (i % 2 == 0) {
				raceList.append("</tr>");
			}
		}
		raceList.append("</table>");
