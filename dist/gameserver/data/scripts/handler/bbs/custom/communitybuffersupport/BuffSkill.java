package handler.bbs.custom.communitybuffersupport;
...
public class BuffSkill {
	...
	public static BuffSkill makeBuffSkill(int id, int level, int subLevel, double timeModifier, int timeAssign, boolean premium) {
		if(level <= 0) {
		...
		Skill skill = SkillHolder.getInstance().getSkill(id, SkillUtils.getSkillLevelMask(level, subLevel));
		if(skill == null) {
			LOGGER.warn("BuffSkill: Error while make buff skill. Cannot find skill ID[" + id + "], LEVEL[" + level + "], SUB_LEVEL[" + subLevel + "]!");
			return null;
		}
