...
public class FreyaStandNormal extends Fighter
{
	...
	protected void thinkAttack()
	{
		...
		{
			actor.doCast(SkillEntry.makeSkillEntry(SkillEntryType.NONE, Skill_EternalBlizzard, 1), actor, true);
		...
		// Ice Ball Cast
		if(!actor.isCastingNow() && !actor.getMovement().isMoving() && _iceballReuseTimer < System.currentTimeMillis())
		{
  			if(topDamager != null && !topDamager.isDead() && topDamager.isInRangeZ(actor, 1000))
			{
				actor.doCast(SkillEntry.makeSkillEntry(SkillEntryType.NONE, Skill_IceBall, 1), topDamager, true);
		...
		{
			actor.doCast(SkillEntry.makeSkillEntry(SkillEntryType.NONE, Skill_SummonElemental, 1), actor, true);
			for(NpcInstance guard : getActor().getAroundNpc(800, 100))
				guard.altOnMagicUse(guard, SkillEntry.makeSkillEntry(SkillEntryType.NONE, Skill_SummonElemental, 1));
		...
		{
			actor.doCast(SkillEntry.makeSkillEntry(SkillEntryType.NONE, Skill_SelfNova, 1), actor, true);
		...
		//{
		//	actor.doCast(SkillEntry.makeSkillEntry(SkillEntryType.NONE, Skill_ReflectMagic, 1), actor, true);
		...
		{
			actor.doCast(SkillEntry.makeSkillEntry(SkillEntryType.NONE, Skill_IceStorm, 1), actor, true);
			_icestormReuseTimer = System.currentTimeMillis() + _icestormReuseDelay * 1000L;
		}

		// Death Sentence
		if(!actor.isCastingNow() && !actor.getMovement().isMoving() && _deathsentenceReuseTimer < System.currentTimeMillis())
		{
			if(randomHated != null && !randomHated.isDead() && randomHated.isInRangeZ(actor, 1000))
			{
				actor.doCast(SkillEntry.makeSkillEntry(SkillEntryType.NONE, Skill_DeathSentence, 1), randomHated, true);
				_deathsentenceReuseTimer = System.currentTimeMillis() + _deathsentenceReuseDelay * 1000L;
			}
		}

		// Freya Anger
		if(!actor.isCastingNow() && !actor.getMovement().isMoving() && _angerReuseTimer < System.currentTimeMillis())
		{
			actor.doCast(SkillEntry.makeSkillEntry(SkillEntryType.NONE, Skill_Anger, 1), actor, true);
