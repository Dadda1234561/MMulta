...
public class AndreasVanHalter extends Fighter
{
	...
	private class Movie extends RunnableImpl
	{
		...
		public void runImpl()
		{
			...
			switch(_taskId)
			{
				...
				case 2:
					// Ищем заного жертву
					NpcInstance npc2 = GameObjectsStorage.getNpc(RitualOffering);
				...
				case 3:
					// Удаляем жертву
					NpcInstance npc3 = GameObjectsStorage.getNpc(RitualOffering);
					npc3.deleteMe();
					// Ищем игроков в радиусе и показываем мувик
					for(Player player : World.getAroundPlayers(actor))
					{
	...
	private void DeleteNpc()
	{
		...
		{
			// Ищем во всем мире
			NpcInstance npc = GameObjectsStorage.getNpc(npcId);
			if(npc != null)
				npc.deleteMe();
		}
