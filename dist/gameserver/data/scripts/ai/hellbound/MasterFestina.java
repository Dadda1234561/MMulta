...
public class MasterFestina extends Fighter
{
	...
	protected void onEvtSpawn()
	{
		...
		{
			MonsterInstance mob = new MonsterInstance(IdFactory.getInstance().getNextId(), NpcHolder.getInstance().getTemplate(FOUNDRY_MYSTIC_ID), StatsSet.EMPTY);
		...
		{
			MonsterInstance mob = new MonsterInstance(IdFactory.getInstance().getNextId(), NpcHolder.getInstance().getTemplate(FOUNDRY_SPIRIT_GUARD_ID), StatsSet.EMPTY);
			mob.setSpawnedLoc(loc);
			mob.setReflection(actor.getReflection());
			mob.setCurrentHpMp(mob.getMaxHp(), mob.getMaxMp(), true);
			mob.spawnMe(mob.getSpawnedLoc());
