	...
	protected boolean thinkActive() {
		...
		if(actor.getDistance(target) < 100 || currentState == 0 || currentState >= coords.length) {
			if(currentState < coords.length) {
				actor.getMovement().moveToLocation(coords[currentState][0], coords[currentState][1], coords[currentState][2], Rnd.get(0, 50), true);
				if(actor.getMovement().getDestination() == null) {
					++currentState;
				}
			} else {
				...
				&& actor.getMovement().getDestination() == null) {
