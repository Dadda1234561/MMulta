...
public class WrapTraps extends DefaultAI
{
	...
	private void timerS(Creature player)
	{
	    ...
	    {
		    skillEntry = SkillEntry.makeSkillEntry(SkillEntryType.NONE, Rnd.get(trapSkills), _level);
		    getActor().doCast(skillEntry, player, true);
		    _period = 8000;
	    }
		else if(_type == 2)
		{
			skillEntry = SkillEntry.makeSkillEntry(SkillEntryType.NONE, 16413, _level);
			getActor().doCast(skillEntry, player, true);
			_period = 5000;
		}
		else if(_type == 3)
		{
			skillEntry = SkillEntry.makeSkillEntry(SkillEntryType.NONE, 16379, 1);
			getActor().doCast(skillEntry, player, true);
			_period = 10000;
		}
	}
