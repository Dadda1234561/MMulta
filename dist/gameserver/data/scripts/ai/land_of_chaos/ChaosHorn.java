...
public class ChaosHorn extends Fighter
{
	...
	private static final SkillEntry CHAOTIC_REIGN_SKILL = SkillEntry.makeSkillEntry(SkillEntryType.NONE, 15569, 1);	// Chaotic Reign
			...
			else if(_type == HornType.GREEN)
			{
				if(_hitCount >= 5)
				{
					...
					for(NpcInstance npc : World.getAroundNpc(getActor(), 1000, 200))
					{
						...
						Location loc = territory.getRandomLoc(getActor().getGeoIndex(), false);
	...
	protected void onEvtTimer(int timerId, Object arg1, Object arg2)
	{
		...
		else if(timerId == NORMAL_HORN_WALK_TIMER_ID)
		{
			...
			getActor().getMovement().moveToLocation(territory.getRandomLoc(getActor().getGeoIndex(), false), 0, true);
		}
	}
}