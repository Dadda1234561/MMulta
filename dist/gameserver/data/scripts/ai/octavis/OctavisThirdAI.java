...
public class OctavisThirdAI extends Fighter
{
	...
	protected void onEvtAttacked(Creature attacker, Skill skill, int damage)
	{
		if(Rnd.chance(3))
		{
			...
			if(Rnd.chance(50))
			{
				volcano.doCast(SkillEntry.makeSkillEntry(SkillEntryType.NONE, OCTAVIS_POWER1_SKILL_ID, 1), volcano, true);

				List<NpcInstance> octavisPowers = reflection.getNpcs(true, OCTAVIS_INFLUENCE_NPC_ID);
				for(NpcInstance octavisPower : octavisPowers)
					octavisPower.setNpcState((octavisPower.getNpcState() + 1) % 7);
			}
			else
				volcano.doCast(SkillEntry.makeSkillEntry(SkillEntryType.NONE, OCTAVIS_POWER2_SKILL_ID, 1), volcano, true);
		}
