...
public class OctavisBeastAI extends Fighter
{
	...
	protected void onEvtSeeSpell(Skill skill, Creature caster, Creature target)
	{
		...
		if(skillId == CHAIN_STRIKE_SKILL_ID || skillId == CHAIN_HYDRA_SKILL_ID)
		{
			if(Rnd.chance(40))
			{
				SkillEntry castSkillEntry;
				if(Rnd.chance(50))
					castSkillEntry = SkillEntry.makeSkillEntry(SkillEntryType.NONE, BEAST_HERO_MOVEMENT_SKILL_ID, 1);
				else
					castSkillEntry = SkillEntry.makeSkillEntry(SkillEntryType.NONE, BEAST_ANCIENT_POWER_SKILL_ID, 1);

				getActor().forceUseSkill(castSkillEntry, caster);
			}
		}
	}
}