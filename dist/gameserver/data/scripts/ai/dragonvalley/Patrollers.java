...
public class Patrollers extends Fighter
{
	...
	{
		if(!getActor().getMovement().isMoving() && getIntention() == CtrlIntention.AI_INTENTION_ACTIVE)
			startMoveTask();
...
//		if(Rnd.chance(30))
//			npc.altOnMagicUse(npc, SkillEntry.makeSkillEntry(SkillEntryType.NONE, 6757, 1));
		...
		if(npc instanceof MonsterInstance)
		{
			...
			if(_monster.getMinionList() != null && _monster.getMinionList().hasMinions())
			{
				for(NpcInstance _npc : _monster.getMinionList().getAliveMinions())
				{
					...
					_npc.getAI().addTaskMove(loc, true, false);
				}
			}
		}
		doTask();
