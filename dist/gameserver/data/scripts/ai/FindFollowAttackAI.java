...
public class FindFollowAttackAI extends Fighter
{
	...
	private boolean startAttack()
	{
		...
		if (!getActor().getAroundCharacters(300, 150).isEmpty() && getActor().getMovement().getFollowTarget() == null)
		{
			for (Creature obj : getActor().getAroundCharacters(300, 150))
			{
				if (obj.isPlayer())
					actor.getMovement().setFollowTarget(obj);
			}
		}
		...
		if(_target == null && !attack)
		{
			...
			else if(!actor.isInRange(target, Config.FOLLOW_RANGE + 200) && (!actor.getMovement().isFollow() || actor.getMovement().getFollowTarget() != target))
			{
		...
		{
			if(_target != null && !_target.isDead() && GeoEngine.canSeeTarget(actor, _target))
			{
