...
public class SpaciaBoss extends Fighter
{
	...
	public class SpawnMinion implements Runnable
	{
		@Override
		public void run()
		{
			...
			if(getActor().getReflection() instanceof SpaciaNormal)
			{
				...
				for(Player p : actor.getReflection().getPlayers())
					for(NpcInstance minion : actor.getReflection().getNpcs(true, MinionNormal))
			...
			else
			{
				...
				for(Player p : actor.getReflection().getPlayers())
					for(NpcInstance minion : actor.getReflection().getNpcs(true, MinionHard))
						minion.getAI().notifyEvent(CtrlEvent.EVT_AGGRESSION, p, 2);
			}
		}
	}
