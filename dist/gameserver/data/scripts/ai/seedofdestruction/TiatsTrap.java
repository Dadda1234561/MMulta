	...
	protected boolean thinkActive()
	{
		...
		if(!actor.getAroundCharacters(200, 150).isEmpty())
		{
			...
			if(ArrayUtils.contains(holdTraps, actor.getNpcId()))
				skillEntry = SkillEntry.makeSkillEntry(SkillEntryType.NONE, 4186, 9);
			else if(ArrayUtils.contains(damageTraps, actor.getNpcId()))
				skillEntry = SkillEntry.makeSkillEntry(SkillEntryType.NONE, 5311, 9);
			else if(ArrayUtils.contains(stunTraps, actor.getNpcId()))
				skillEntry = SkillEntry.makeSkillEntry(SkillEntryType.NONE, 4072, 10);
			else
				return false;
