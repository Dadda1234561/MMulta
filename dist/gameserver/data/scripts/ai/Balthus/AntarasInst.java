...
{
	// debuffs
	final SkillEntry s_antaras_lite_fear = getSkill(14387, 1), s_antaras_lite_short_fear1 = getSkill(14388, 1),
			s_antaras_lite_debuff = getSkill(14381, 1),  s_antaras_lite_breath = getSkill(14384, 1);

	// AOE skills
	final SkillEntry s_antaras_lite_jump = getSkill(14382, 1), s_antaras_lite_meteor1 = getSkill(14383, 1);

	//ordinal
	final SkillEntry s_antaras_lite_mouth = getSkill(14378, 1), s_antaras_lite_mouth2 = getSkill(14379, 1),
		...
		if(r != null)
		{
			if (r instanceof BaltusKnight)
			{
				...
				else if (lInst.getStage() == 2)
				{
					...
					addTaskMove(Location.findPointToStay(INCAVE, 40, 40, getActor().getGeoIndex()), true, false);
				}
				else if (lInst.getStage() == 4)
				{
					getActor().getAggroList().clear();
					addTaskMove(Location.findPointToStay(CENTER, 40, 40, getActor().getGeoIndex()), true, false);
				}
				else if (lInst.getStage() == 11)
				{
					getActor().getAggroList().clear();
					addTaskMove(Location.findPointToStay(SPAWN, 40, 40, getActor().getGeoIndex()), true, false);
				}
			}
		}
