...
public class BaiumManager implements OnInitScriptListener
{
	...
	private final static Location STATUE_LOCATION = new Location(115752, 17160, 10080, 41740);
	...
	private synchronized static void setIntervalEndTask()
	{
		...
		if (_state.getState().equals(State.INTERVAL) && remainingTime > THIRTY_MIN_IN_MILLIS)
		{
			...
			// schedule new
			_announceTask = ThreadPoolManager.getInstance().schedule(() -> Announcements.announceToAllFromStringHolder("custom.CustomEpicBoss.PreSpawn", "Baium", "30"), announceDelay);
			_log.info("BaiumManager: Scheduled pre-spawn announce to " + TimeUtils.toSimpleFormat(System.currentTimeMillis() + announceDelay));
		}
		else
		{
			_log.info("BaiumManager: Not scheduling pre-spawn announce to as time to spawn is " + TimeUtils.toSimpleFormat(System.currentTimeMillis() + remainingTime) + ". [" + Util.formatTime((int) (remainingTime / 1000)) + " ]");
		}
