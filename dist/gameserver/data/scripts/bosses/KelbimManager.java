...
public class KelbimManager implements OnInitScriptListener
{
	...
	private static final int FWA_FIXINTERVALOFKELBIM = 1 * 32 * 60 * 60000; // 7 суток +-
	...
	private static class KelbimSpawn implements Runnable
	{
	...
	public static void enterTheCastle(Player /*ccleader*/player)
	{
		if(player == null)
			return;
		...
		if(_state.getState() == EpicBossState.State.ALIVE)
		{
			...
			if(getKelbimStage() >= 2 && (!player.isDead() || !player.isFlying() || !player.isCursedWeaponEquipped() || player.getLevel() == 85))
			{
