...
public class FightClubManagerInstance extends NpcInstance
{
	...
	private String makeAcceptHtml(Player player, int storedId)
	{
		final Rate rate = FightClubManager.getRateByStoredId(storedId);
		String html = HtmCache.getInstance().getHtml(HTML_ACCEPT, player);
		html = html.replace("%name%", rate.getPlayerName());
		html = html.replace("%class%", rate.getPlayerClass().getName(player));
		html = html.replace("%level%", String.valueOf(rate.getPlayerLevel()));
		html = html.replace("%rate%", rate.getItemCount() + " " + rate.getItemName());
		html = html.replace("%storedId%", String.valueOf(rate.getStoredId()));
		return html;
	}
