		...
		if(ask == 4378)
		{
			...
			if(reply == 1)
			{
				...
				else if(player.hasPremiumAccount() || !Config.TRAINING_CAMP_PREMIUM_ONLY)
				{
					...
					trainingCamp = new TrainingCamp(player.getAccountName(), player.getObjectId(), player.getActiveDualClass().getIndex(), player.getLevel(), System.currentTimeMillis());
			...
			else if(reply == 2)
			{
				...
				{
					if(trainingCamp.getClassIndex() == player.getActiveDualClass().getIndex())
					{
