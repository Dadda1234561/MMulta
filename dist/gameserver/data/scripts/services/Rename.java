...
public class Rename
{
	...
	public void separate_page(Player player, NpcInstance npc, String[] param)
	{
		...
		if(player.getDualClassList().size() == 1)
		{
			Functions.show("You must have dualclass.", player);
			return;
		}
		...
		if(player.getActiveDualClass().getLevel() < 85)
		{
			Functions.show("You must have at least 85 level.", player);
			return;
		}
		...
		for(DualClass d : player.getDualClassList().values())
			if(!d.isBase())
				append += "<tr><td><button value=\"" + new CustomMessage("scripts.services.Separate.Button").addString(ClassId.VALUES[d.getClassId()].toString()).toString(player) + "\" action=\"bypass -h htmbypass_services.Rename:separate " + d.getClassId() + " $name\" width=200 height=15 back=\"L2UI_CT1.Button_DF_Down\" fore=\"L2UI_CT1.Button_DF\"></td></tr>";
	...
	public void separate(Player player, NpcInstance npc, String[] param)
	{
		...
		if(mysql.simple_get_int("level", "character_dualclasses", "char_obj_id=" + newcharid + " AND level > 1") > 1)
		{
			Functions.show("Target must have level 1.", player);
			return;
		}

		mysql.set("DELETE FROM character_dualclasses WHERE char_obj_id=" + newcharid);
		...
		mysql.set("UPDATE character_dualclasses SET char_obj_id=" + newcharid + ", isBase=1, dual_certification=0 WHERE char_obj_id=" + player.getObjectId() + " AND class_id=" + classtomove);
		...
		player.modifyDualClass(classtomove, -1, false);
	...
	public void changebase_page(Player player, NpcInstance npc, String[] param)
	{
		...
		List<DualClass> possible = new ArrayList<DualClass>();
		if(player.getActiveDualClass().isBase())
		{
			possible.addAll(player.getDualClassList().values());
			possible.remove(player.getDualClassList().getByClassId(player.getBaseClassId()));

			for(DualClass d : player.getDualClassList().values())
				for(DualClass d2 : player.getDualClassList().values())
					if(d != d2 && !DualClassTable.areClassesComportable(ClassId.VALUES[d.getClassId()], ClassId.VALUES[d2.getClassId()]) || d2.getLevel() < 85)
						possible.remove(d2);
		}
		...
		if(possible.isEmpty())
			...
			for(DualClass d : possible)
				append += "<tr><td><button value=\"" + new CustomMessage("scripts.services.BaseChange.Button").addString(ClassId.VALUES[d.getClassId()].toString()).toString(player) + "\" action=\"bypass -h htmbypass_services.Rename:changebase " + d.getClassId() + "\" width=200 height=15 back=\"L2UI_CT1.Button_DF_Down\" fore=\"L2UI_CT1.Button_DF\"></td></tr>";
		append += "</table>";
		Functions.show(append, player);
	}
	...
	public void changebase(Player player, NpcInstance npc, String[] param)
	{
		...
		DualClass newBase = player.getDualClassList().getByClassId(target);

		player.getActiveDualClass().setType(DualClassType.DUAL_CLASS);
		player.getActiveDualClass().setDualCertification(newBase.getDualCertification());

		newBase.setDualCertification(0);
		player.getActiveDualClass().setExp(player.getExp(), false);
		player.checkSkills();

		newBase.setType(DualClassType.BASE_CLASS);
	...
	public void rename_clan(Player player, NpcInstance npc, String[] param)
	{
		...
		player.getClan().setName(param[0], true);
