...
public class CorallGarden extends Reflection
{
	...
	private class MonsterMovementTask implements Runnable
	{
		...
		public void run()
		{
			if(golem1 != null)
			{
				if(golem1.getMovement().getFollowTarget() != null && (golem1.getMovement().getFollowTarget().getTarget() != golem1 || golem1.getDistance(golem1.getMovement().getFollowTarget()) > 200))
				{
					CrystalGolem ai = (CrystalGolem) golem1.getAI();
					golem1.setRunning();
					ai.addTaskMove(Location.findPointToStay(golem1.getLoc(), 100, 100, golem1.getGeoIndex()), true, false);
					countoffear1++;
					if(countoffear1 > 4)
					{
						golem1.deleteMe();
				...
				else if(golem1.getMovement().getFollowTarget() != null && golem1.getMovement().getFollowTarget().getTarget() == golem1 && golem1.getDistance(golem1.getMovement().getFollowTarget()) < 200)
				{
					if (getStatus() == 0)
					{
						if (!golem1.getMovement().isMoving() && golem1.getAI().getIntention() != CtrlIntention.AI_INTENTION_CAST && golem1.getAI().getIntention() != CtrlIntention.AI_INTENTION_ATTACK)
						{
							...
							if (nearestLoc != null)
							{
								if(nearestLoc == routes.get(16))
								{
									...
									golem1.getMovement().setFollowTarget(null);
									firstgolem = true;
								...
								Creature ftar = golem1.getMovement().getFollowTarget();
								nearestLoc.setX(nearestLoc.getX());
								nearestLoc.setY(nearestLoc.getY());
								golem1.setWalking();
								golem1.setBusy(true);
								CrystalGolem ai = (CrystalGolem) golem1.getAI();
								golem1.getMovement().setFollowTarget(ftar);
								ai.addTaskMove(Location.findPointToStay(nearestLoc, 1, 1, golem1.getGeoIndex()), true, false);
							}
						}
					}
				}
			...
			if(golem2 != null)
			{
				if(golem2.getMovement().getFollowTarget() != null && golem2.getMovement().getFollowTarget().getTarget() != golem2 && golem2.getDistance(golem2.getMovement().getFollowTarget()) > 200)
				{
					CrystalGolem ai = (CrystalGolem) golem2.getAI();
					ai.addTaskMove(Location.findPointToStay(golem2.getLoc(), 100, 100, golem2.getGeoIndex()), true, false);
					golem2.setRunning();
					countoffear2++;
					if(countoffear2 > 4)
					{
				...
				else if(golem2.getMovement().getFollowTarget() != null && golem2.getMovement().getFollowTarget().getTarget() == golem2 && golem2.getDistance(golem2.getMovement().getFollowTarget()) < 200)
				{

					if (getStatus() == 0)
					{
						if (!golem2.getMovement().isMoving() && golem2.getAI().getIntention() != CtrlIntention.AI_INTENTION_CAST && golem2.getAI().getIntention() != CtrlIntention.AI_INTENTION_ATTACK)
						{
							List<Location> routes;
							...
							if (nearestLoc != null)
							{
								if(nearestLoc == routes.get(16))
								{
									...
									golem2.getMovement().setFollowTarget(null);
									secondgolem = true;
									if(firstgolem && secondgolem && getStatus() == 0)
									{
										spawnrb();
								...
								Creature ftar = golem2.getMovement().getFollowTarget();
								nearestLoc.setX(nearestLoc.getX());
								nearestLoc.setY(nearestLoc.getY());
								golem2.setWalking();
								golem2.setBusy(true);
								CrystalGolem ai = (CrystalGolem) golem2.getAI();
								golem2.getMovement().setFollowTarget(ftar);
								ai.addTaskMove(Location.findPointToStay(nearestLoc, 1, 1, golem2.getGeoIndex()), true, false);
							}
						}
					}
				}
	...
	{
		for(NpcInstance mob : getNpcs(true, npcId))
			mob.deleteMe();
	}
	...
	public void spawnMinions(int npcId)
	{
		...
		if(!players.isEmpty())
		{
			for(NpcInstance minion : minions)
			{
				...
				ai.addTaskMove(Location.findPointToStay(Rnd.get(players).getLoc(), 1, 1, minion.getGeoIndex()), true, false);
			}
		}
	}
}