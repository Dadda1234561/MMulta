...
public class HeartInfinityAttack extends Reflection
{
	...
	private void conquestBegins()
	{
		...
		for(NpcInstance n : getNpcs(true, AliveTumor))
	...
	{
		return getNpcs(true, AliveTumor).size();
	}
	...
	private void handleEkimusStats()
	{
		...
		ekimus.getStat().removeFuncsByOwner(this);
		ekimus.getStat().addFuncs(new FuncSet(Stats.POWER_ATTACK, 0x30, this, ekimus.getBaseStats().getPAtk() * 3, StatsSet.EMPTY)); // constant
		ekimus.getStat().addFuncs(new FuncSet(Stats.MAGIC_ATTACK, 0x30, this, ekimus.getBaseStats().getMAtk() * 10, StatsSet.EMPTY)); // constant
		ekimus.getStat().addFuncs(new FuncSet(Stats.POWER_DEFENCE, 0x30, this, ekimus.getBaseStats().getPDef() * a[1], StatsSet.EMPTY));
		ekimus.getStat().addFuncs(new FuncSet(Stats.MAGIC_DEFENCE, 0x30, this, ekimus.getBaseStats().getMDef() * a[0], StatsSet.EMPTY));
		ekimus.getStat().addFuncs(new FuncSet(Stats.REGENERATE_HP_RATE, 0x30, this, ekimus.getBaseStats().getHpReg() * a[2], StatsSet.EMPTY));
	}
