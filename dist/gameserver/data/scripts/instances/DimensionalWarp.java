...
public class DimensionalWarp extends Reflection
{
	...
	private void startCollapseTask()
	{
		if(_collapsetask != null)
			_collapsetask.cancel(false);
		_collapsetask = ThreadPoolManager.getInstance().schedule(() -> startCollapseTimer(0, true), _instanceEndTime - System.currentTimeMillis());
	}
	...
	private class SpawnTrapsTask implements Runnable
	{
		@Override
		public void run()
		{
	...
	private class DebufTask implements Runnable
	{
		@Override
		public void run()
		{
			if(getStage() > 11 && getStage() <= 20)
				...
				_skilllevel = 4;
			SkillEntry skill = SkillEntry.makeSkillEntry(SkillEntryType.NONE, 16415, _skilllevel);
			for (Player p : getPlayers())
			{
