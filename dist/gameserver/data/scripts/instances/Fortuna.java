...
{
	private class FortunaProcessTask implements Runnable
	{
		@Override
		public void run()
		{
			...
			else if(_state ==  FortunaState.SECOND_STAGE)
			{
				...
				{
					List<NpcInstance> npcs = getNpcs(false, CELLPHINE);
		...
		public void onDeath(Creature actor, Creature killer)
		{
			if(actor.isMonster())
			{
				...
				else if(_state ==  FortunaState.SEVENTH_STAGE)
				{
					...
					{
						if(getNpcs(true, MUKSHU_THE_COWARD_RAID_BOSS).isEmpty())
							startProcessTask(FortunaState.EIGHT_STAGE, 1, 30000L);
					}
					else if(npcId == MUKSHU_THE_COWARD_RAID_BOSS)
					{
						if(getNpcs(true, BLIND_HORNAFI_RAID_BOSS).isEmpty())
							startProcessTask(FortunaState.EIGHT_STAGE, 1, 30000L);
					}
				}
				else if(_state ==  FortunaState.EIGHT_STAGE)
