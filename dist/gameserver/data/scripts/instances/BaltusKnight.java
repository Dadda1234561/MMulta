...
public class BaltusKnight extends Reflection
{
	...
	public class AntarasSpawn implements Runnable
	{
		Reflection _r;
		...
		public void run()
		{
			...
			ThreadPoolManager.getInstance().schedule(() ->
			{
				...
				Eiteld.setRunning();
				ai.addTaskMove((Location.findPointToStay(175544, 115992, -7733, 40, 40, Eiteld.getGeoIndex())), false, false);
			}, 48000);
			...
			ThreadPoolManager.getInstance().schedule(() ->
			{
				...
				for(NpcInstance npc : getNpcs(true, 19133))
					npc.setBusy(true);
				for(NpcInstance npc : getNpcs(true, 19136))
					npc.setBusy(true);
				Faula.setBusy(true);
				Eiteld.setBusy(true);
				Felo.setBusy(true);
	...
	private class MonsterMovementTask implements Runnable
	{
		...
		public void run()
		{
			if(_antharas != null && !_antharas.isDead())
			{
				...
				{
					if(!_antharas.getMovement().isMoving() && _antharas.getAI().getIntention() != CtrlIntention.AI_INTENTION_CAST)
					{
						...
						if (_stage == 1)
						{
							ai.addTaskMove(Location.findPointToStay(CENTER, 40, 40, _antharas.getGeoIndex()), true, false);
						...
						if (_stage == 2)
						{
							ai.addTaskMove(Location.findPointToStay(INCAVE, 40, 40, _antharas.getGeoIndex()), true, false);
								...
								ThreadPoolManager.getInstance().schedule(() ->
								{
									for(NpcInstance bombers : getNpcs())
									{
										...
										if(bombers.getNpcId() ==  29227)
											bombers.getAI().getActor().doCast(SkillEntry.makeSkillEntry(SkillEntryType.NONE, 14390, 1), bombers.getAI().getActor(), true);
									}
								}, 10000L);
							}
						}
						if (_stage == 11)
						{
							ai.addTaskMove(Location.findPointToStay(Antharasspawn, 40, 40, _antharas.getGeoIndex()), true, false);
							...
							if (curloc <= 350)
							{
								...
								{
									_antharas.deleteMe();
									for(NpcInstance npc : getNpcs(true, 29230))
										npc.deleteMe();
								}, 1500);
