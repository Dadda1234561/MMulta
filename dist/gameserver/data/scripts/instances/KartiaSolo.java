...
public class KartiaSolo extends Kartia
{
	private class LifeThiefDeathListener implements OnDeathListener
	{
		...
		public void onDeath(Creature victim, Creature killer)
		{
			...
			{
				SkillEntry skill = SkillEntry.makeSkillEntry(SkillEntryType.NONE, 15401, 1);
				if(skill != null)
				{
	...
	private class KartiaSupportTask implements Runnable
	{
		@Override
		public void run()
		{
			...
			for(NpcInstance follower : _followers)
			{
				...
				if(getPlayers() != null && !getPlayers().isEmpty())
					follower.getMovement().setFollowTarget(getPlayers().get(0));
		...
		public void run()
		{
			...
			if(_healer != null)
			{
				...
				if(percentHp <= 0.5D)
				{
					...
					_healer.doCast(SkillEntry.makeSkillEntry(SkillEntryType.NONE, 14899, 1), player, true);
					...
					if(needTree)
					{
						ThreadPoolManager.getInstance().schedule(new Runnable()
						{
							...
							public void run()
							{
								_healer.doCast(SkillEntry.makeSkillEntry(SkillEntryType.NONE, 14903, 1), player, true);
	...
	static
	{
		FIRST_STATE_WALKER_ROUTE.addPoint(new WalkerRoutePoint(new Location(-111320, -10456, -11710), new NpcString[0], -1, 5, true, false)); // На оффе в этой точке они тупят 5 секунд.
		FIRST_STATE_WALKER_ROUTE.addPoint(new WalkerRoutePoint(new Location(-110328, -10456, -11949), new NpcString[0], -1, 0, true, false));
	}
	...
	public boolean deselectSupport(int support)
	{
	...
	{
		SkillEntry buff = SkillEntry.makeSkillEntry(SkillEntryType.NONE, 15003, 1);
		SkillEntry heal = SkillEntry.makeSkillEntry(SkillEntryType.NONE, 15002, 1);
		...
		ThreadPoolManager.getInstance().schedule(new Runnable()
		{
			...
			public void run()
			{
				if((tree != null) && (!player.isDead()))
				{
					...
					tree.doCast(SkillEntry.makeSkillEntry(SkillEntryType.NONE, 15002, 1), player, true);
		...
		ThreadPoolManager.getInstance().schedule(new Runnable(){
			...
			public void run(){
				if((tree != null) && (!player.isDead())){
					...
					tree.doCast(SkillEntry.makeSkillEntry(SkillEntryType.NONE, 15003, 1), player, true);
