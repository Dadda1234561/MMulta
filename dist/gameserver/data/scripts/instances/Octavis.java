		...
		switch(_state)
		{
			...
			case FIRST_STAGE:
			{
				...
				ThreadPoolManager.getInstance().schedule(() ->
				{
					...
					List<NpcInstance> gladiators = getNpcs(true, OCTAVIS_GLADIATOR);
					for(NpcInstance gladiator : gladiators)
					{
						...
						gladiator.getMovement().moveToLocation(loc, 0, true);
					}
					...
					ThreadPoolManager.getInstance().schedule(new Runnable()
					{
						...
						public void run()
						{
							if(_state == OctavisState.SECOND_STAGE)
							{
								...
								{
									List<NpcInstance> octavises = getNpcs(true, _hard ? OCTAVIS_HARD_SECOND : OCTAVIS_LIGHT_SECOND);
									if(!octavises.isEmpty())
									{
										NpcInstance octavis = octavises.get(0);
										MinionSpawner spawner = octavis.getMinionList().addMinion(ARENA_BEAST, null, 1, 0);
										if(spawner != null)
										{
