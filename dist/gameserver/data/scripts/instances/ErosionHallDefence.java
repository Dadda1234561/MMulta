...
public class ErosionHallDefence extends Reflection
{
	...
	private void conquestBegins()
	{
		...
		{
			if(!conquestEnded)
				for(NpcInstance npc : getNpcs(true, DeadTumor))
					spawnCoffin(npc);
		}, 1000L, 60000L);
	...
	private class DeathListener implements OnDeathListener
	{
		...
		public void onDeath(Creature self, Creature killer)
		{
			...
			{
				if(getNpcs(true, SoulWagon).size() > 0)
			...
			if(seed != null)
			{
				if(npc.getAI().getIntention() == CtrlIntention.AI_INTENTION_ACTIVE)
				{
					...
					ThreadPoolManager.getInstance().schedule(() ->
					{
						...
						npc.getMovement().moveToLocation(Location.findAroundPosition(npc, 400), 0, false);
					}, 7000L);
				}
			}
		}
	...
	{
		for(NpcInstance npc : getNpcs(true, AliveTumor))
	...
	private void notifyTumorDeath()
	{
		...
		if(tumorKillCount > 4 && !soulwagonSpawned) // 16
		{
			...
			for(NpcInstance npc : getNpcs(true, SoulWagon))
			{
