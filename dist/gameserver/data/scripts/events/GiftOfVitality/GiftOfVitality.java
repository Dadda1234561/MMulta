...
public class GiftOfVitality implements OnInitScriptListener
{
	...
	private void spawnEventManagers()
	{
		...
		_spawns.add(NpcUtils.spawnSingle(EVENT_MANAGER_ID, 82145, 148609, -3464, 0, 0)); //Giran
		...
		switch(type)
		{
			case VITALITY:
				...
				else
				{
					...
					player.altOnMagicUse(player, SkillEntry.makeSkillEntry(SkillEntryType.NONE, 23179, 1));
				...
				else
				{
					for(Servitor s : player.getServitors())
					{
						for(int[] buff : _summonBuff)
						{
							...
							player.altOnMagicUse(s, SkillEntry.makeSkillEntry(SkillEntryType.NONE, buff[0], buff[1]));
						}
					}
				...
				else
				{
					if(!player.isMageClass() || player.getTemplate().getRace() == Race.ORC)
						for(int[] buff : _warrBuff)
						{
							...
							player.altOnMagicUse(player, SkillEntry.makeSkillEntry(SkillEntryType.NONE, buff[0], buff[1]));
						}
					else
