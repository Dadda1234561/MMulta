...
public class _10823_ExaltedOneWhoShattersTheLimit extends Quest
{
	private static class ChangeLevelListener implements OnLevelChangeListener
	{
		...
		{
			QuestState qs = player.getQuestState(10823);
			if(qs == null)
				return;

			if ((qs.getCond() == 1) && (qs.getQuestItemsCount(PROOF_OF_PREPARATION) >= 40000) && (player.getLevel() >= 103))
				qs.setCond(2);
		}
	}
	...
	public _10823_ExaltedOneWhoShattersTheLimit()
	{
		...
		addQuestCompletedCheck("no_level.htm", 10817);
	}
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		switch (event)
		{
			case "lionel_hunter_q10823_03.htm":
			case "lionel_hunter_q10823_04.htm":
			{
				break;
			}
			case "lionel_hunter_q10823_05.htm":
			{
				st.setCond(1);
				break;
			}
			case "lionel_hunter_q10823_08.htm":
			{
				if (st.getCond() == 2)
				{
					player.setNobleType(NobleType.HONORABLE);
					st.takeAllItems(PROOF_OF_PREPARATION);
					st.giveItems(EXALTED_CLOAK, 1);
					st.giveItems(EXALTED_TIARA, 1);
					st.giveItems(DIGNITY_OF_THE_EXALTED, 1);
					st.giveItems(SUMMON_BATTLE_POTION, 1);
					st.giveItems(FAVOR_OF_THE_EXALTED, 1);
					st.giveItems(FATE_OF_THE_EXALTED, 1);
					st.giveItems(BLESSING_OF_THE_EXALTED, 1);
					st.finishQuest();
					break;
				}
			}
	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		switch (npcId)
		{
			case LIONEL:
			{
				...
				{
					case 0:
						return "lionel_hunter_q10823_01.htm";
					case 1:
						return "lionel_hunter_q10823_06.htm";
					case 2:
						return "lionel_hunter_q10823_07.htm";
				}
				break;
			}
		}
	...
	{
		if ((qs.getCond() == 1) && (qs.getQuestItemsCount(PROOF_OF_PREPARATION) < 40000) && ArrayUtils.contains(MONSTERS, npc.getNpcId()))
		{
			qs.giveItems(PROOF_OF_PREPARATION, 1);
			if ((qs.getQuestItemsCount(PROOF_OF_PREPARATION) >= 40000) && qs.getPlayer().getLevel() >= 103)
			{
				qs.setCond(2);	
			}
			else
