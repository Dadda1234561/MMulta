...
public class _197_SevenSignsTheSacredBookofSeal extends Quest
{
	...
	private static int SignDawn = 17265;
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		{
			st.setCond(6);
			st.giveItems(MysteriousHandwrittenText, 1, false);
		}
		else if(event.equalsIgnoreCase("wood_q197_4.htm"))
			if(player.getBaseClassId() == player.getActiveClassId())
			{
				st.takeItems(PieceofDoubt, -1);
				st.addExpAndSp(750000000, 75000000);
				st.giveItems(DawnsBracelet, 1, false);
				st.giveItems(SignDawn, 1, false);
				st.finishQuest();
			}
			else
	...
	public String onKill(NpcInstance npc, QuestState st)
	{
		...
		if(npc.getNpcId() == ShilensEvilThoughts && cond == 3)
		{
			...
			st.giveItems(PieceofDoubt, 1, false);
